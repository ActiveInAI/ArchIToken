...
**定稿日期**: 2026-04-19
**2026-05-28 同步**: 当前 active registry 为 **16 modules**; `personal_center`、`settings_center` 为平台级并列模块。详细规范见 [`MODULES.md`](./MODULES.md) · 注册机制见 [`MODULE-REGISTRY.md`](./MODULE-REGISTRY.md).
**2026-05-15 定位同步**: ArchIToken 固定定位为 **AEC AI-Native + Harness Engineering + OpenBIM CDE Workflow OS**。竞品边界与对标策略见 [`POSITIONING_AND_COMPETITIVE_STRATEGY.md`](./POSITIONING_AND_COMPETITIVE_STRATEGY.md).
...
NVIDIA 目标硬件的软件栈必须以 NVIDIA NGC 签名 CUDA / CUDA Deep Learning 镜像、NVIDIA Container Toolkit、GPU Operator / device plugin、DCGM、CUDA、OptiX、TensorRT / TensorRT-LLM 和 Triton 为主路线。Docker / k8s 镜像必须锁定 tag 或 digest 并产生 `nvidia-smi`、`nvcc` CUDA kernel、WebGPU adapter 和渲染/推理 runtime evidence; Mesa、CPU-only、WebGL-only、截图或空 Canvas 只能作为失败证据后的受审计恢复路径,不能作为 GPU 渲染完成证据。
