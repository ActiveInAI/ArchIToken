...
describe("file type registry", () => {
  ...
  it("routes Office, PDF, code, diagrams, GIS, scans, and robotics", () => {
    ...
    expect(fileTypeForExtension(".ifczip")?.viewerKind).toBe("archive");
    expect(fileTypeForExtension(".bcfzip")?.viewerKind).toBe("archive");
    expect(fileTypeForExtension(".7z")?.viewerKind).toBe("archive");
