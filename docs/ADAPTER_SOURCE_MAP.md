...
| Mesh fallback         | `.stl`, `.ply`, `.dae`, `.3dm`, `.skp`                                                              | Three.js/STL/PLY/DAE/3DM source viewer or Blender/FreeCAD/Prengine command worker; SKP->IFC must use a real SKP reader/exporter command (`PRENGINE_SKP_TO_IFC_COMMAND`) or licensed adapter and is never satisfied by GLB; SKP viewing may use a real SKP-to-GLB command, licensed adapter, or bound GLB final fallback; OBJ/FBX are abandoned legacy inputs, not default targets | STL/PLY/DAE/3DM upload viewers, SKP->IFC command boundary, and SKP GLB fallback wired; native deps required |
