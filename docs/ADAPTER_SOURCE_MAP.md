...
| glTF / GLB fallback   | `.glb`, `.gltf`                                                                                     | Three.js viewer from uploaded bytes or worker derivative only when OpenUSD/USDZ/3D Tiles is unavailable and audited                                        | fallback upload viewer supported                                           |
| Mesh fallback         | `.stl`, `.ply`, `.dae`, `.3dm`, `.skp`                                                              | Three.js/STL loader or Blender/FreeCAD mesh conversion worker; OBJ/FBX are abandoned legacy inputs, not default targets                                    | STL upload viewer and Blender worker wired; native deps required           |
