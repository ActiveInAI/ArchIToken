# Zed + Claude Code 集成 · ArchIToken 工作流
...
## 2 · ArchIToken 项目专用工作流

### 2.1 在项目根放 CLAUDE.md

你已经有了 `~/dev/architoken/CLAUDE.md` (本项目交付文件之一)。
...
```
请读 ~/dev/architoken/CLAUDE.md 和 ~/dev/architoken/versions.toml
...
### 3.3 ArchIToken 项目专用 (自定义到 ~/.claude/commands/)
...
读取 ~/dev/architoken/versions.toml · 查找 $ARG 对应的 current 字段 · 返回精确版本号
EOF
...
查 ~/dev/architoken/versions.toml 的 [model_format_policy.nvfp4_locked] 段
列出已有 NVFP4 的模型 + 升级候选
EOF
```
