...
**ArchIToken 增强附录**: [`ARCHITOKEN-CONSTITUTION-ADDENDUM.md`](./ARCHITOKEN-CONSTITUTION-ADDENDUM.md)

---

## 0. 命名与继承关系

ArchIToken 是 InsomeOS 改名后的同一项目,不是两个并行项目。

历史文档、旧路径、旧模块名可以在迁移期继续保留,但新文档、新代码、新产品界面、新部署命名应逐步迁移到 `ArchIToken`。

本文件与 `ARCHITOKEN-CONSTITUTION-ADDENDUM.md` 优先于旧文档中的冲突表述。
...
**CI 执行**: 禁止新增 `BusinessPhase` / `ModuleEnum` / `CREATE TYPE ... AS ENUM` 表达业务扩展点。模块必须走 `ModuleRegistry` / `modules` 表 / Module Schema。
...
- `ARCHITOKEN-CONSTITUTION-ADDENDUM.md`
- `CONSTITUTION.md`
- `MODULES.md`
- `MODULE-REGISTRY.md`
- `ARCHITECTURE.md`
...
**版本**: ArchIToken Constitution · 21 条 · v2.1 · 由 InsomeOS v2.0 宪法演进而来。
