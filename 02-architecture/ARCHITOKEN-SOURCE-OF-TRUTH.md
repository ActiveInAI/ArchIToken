...
ArchIToken 的工程治理以 GitHub 仓库文档为唯一真源。聊天上下文、临时讨论、图片、口头说明都只能作为输入,不能替代仓库文档。`ArchIToken` 是当前产品、仓库、代码库、包名和 API 兼容标识;新用户界面、对外文档和发行物默认使用 `ArchIToken`。
...
`ArchIToken` 是当前产品名、仓库名、代码库名、路径名和包名/API 兼容标识。新 UI、产品文案、发行物、桌面入口和新增文档默认使用 `ArchIToken`;不得引入其它 active 产品身份,除非先更新唯一真源、宪法、模块注册和发布材料。
