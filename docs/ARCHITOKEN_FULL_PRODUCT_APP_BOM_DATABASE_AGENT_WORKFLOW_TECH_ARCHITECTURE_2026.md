...
| AI 正向工程模型与开放格式派生 | AI 的主线不是聊天生成文档,而是把客服/客户/合同/现场输入转成需求参数,生成类型定义、构件实例、属性集、工程模型草稿和 IFC/STL/STEP/USDZ/OpenUSD 格式包,再由模型导出施工图、加工图、构件 BOM、BOQ、材料需求和报价草稿。 | `design.model_elements`, `design.element_types`, `drawing_sheets`, `fabrication_drawings`, `component_bom_lines`, `boq_items`, `quote_lines`, `agent_runs` | 任一报价项、采购件、加工件、工单或验收点必须能回跳到需求、构件实例、IFC GUID、属性集、图纸页、BOM 行、校验报告、审批记录和 Agent 调用。 |
...
| CPU 基础包 + BIM GPU 专项另购 + 网络安全待锁价 | 一期 CPU/存储预算服务于内部试点、CDE/NAS、数据库/CI、Agent 编排、日志审计和备份。CPU 核心物料基线按六节点、NAS 机箱 ¥3,000、每台线材/散热/小配件 ¥1,000 重算为 ¥340,700；加离线备份盘后已确认物料为 ¥355,500；网络/安全/UPS/机柜/布线必须按京东 SKU 和截图另行锁价。BIM GPU 有两种采购方案:方案 A 为 2 张 NVIDIA RTX PRO 6000D Blackwell Server Edition 84GB PCIe,单价 ¥68,000/张,小计 ¥136,000；方案 B 为 4 张 RTX 5090 32GB 双宽涡轮,单价 ¥36,000/张,小计 ¥144,000。 | `5.25 2026-06 硬件选型、网络、安全与预算` | 六节点 CPU 包仍以 2 x 676X + 4 x 658X、每台至少 64GB ECC RDIMM、不含 HA 为边界；`srv-01`、`srv-02`、`srv-03` 等关键节点可按需升到 256GB 或 CPU 升级到 696X；GPU 卡独立列支,上架前必须通过服务器机箱、PCIe、供电、风道、驱动和质保验收。 |
...
| 硬件预算 | 40-50 万预算不能同时覆盖完整 HA 生产机房。 | CPU 基础包、离线备份、BIM GPU 和网络安全分账:当前按 2 台 676X + 4 台 658X 六节点轻量试点包,CPU 核心物料 ¥340,700；加离线备份盘后已确认物料 ¥355,500；网络/安全/UPS/机柜/布线必须京东锁价后再入总价；GPU 方案 A 为 2 张 RTX PRO 6000D 84GB 服务器版,小计 ¥136,000；GPU 方案 B 为 4 张 RTX 5090 32GB 双宽涡轮,小计 ¥144,000；关键节点可按需升 256GB,公网生产和 HA 仍单独立项。 | P0 |
...
| L2 内部生产力 | 30 人团队内部使用,管理 BOM、文件、审批、Agent 草稿和归档 | 不承诺外部客户 SLA,不承载 1 万公网用户 | `srv-01` 到 `srv-06` 六节点轻量试点包,权限、审计、备份、JumpServer、最小监控 |
...
| AI 正向工程模型与员工 Agent 集群 | 架构正本已补齐从客服/客户/合同/现场输入到工程模型草稿、IFC/STL/STEP/USDZ/OpenUSD 格式包、施工/加工图、模型导出 BOM、BOQ/材料/报价、采购/生产/施工/归档的对象链,并定义面向客服、设计、BIM、造价、材料、生产、施工、档案、财务、人力和管理层的岗位 Agent。 | `5.26 AI 生成工程模型、开放格式输出与员工智能体集群`, `tools/generate_architoken_executive_brief_10p.mjs` | 文档架构已对齐;API、数据库迁移、Worker、前端工作台和合同测试仍需拆 P0 任务实现 |
...
| 验收 | 每个造价行必须能追溯到 BOM 行、构件实例/IFC GUID、图纸视图、价格来源和计算公式；报价发布必须走商务/财务审批。 |
...
| 验收 | 任一孪生构件必须能追溯到构件实例、IFC GUID、BOM 行、生产批次、安装记录和验收记录。 |
...
| 元数据域 | metadata_objects, metadata_fields, metadata_lineage_edges, schema_versions, data_quality_issues | `settings_center`, `ai_center` |
| 协同调度域 | workflow_templates, workflow_runs, operation_queue, resource_locks, schedule_windows | `planning_management`, `ai_center`, `settings_center` |
| 价格证据域 | price_books, price_snapshots, price_evidence, supplier_quotes, purchase_lock_records | `quantity_costing`, `material_logistics`, `finance_management` |
...
### 2.5 数据库控制面: 协同、调度、Agent 与元数据

ArchIToken 的数据库不是只保存 BOM 行。PostgreSQL 是结构化真源,必须同时承担“协同管理、任务调度、Agent 运行态、元数据治理、价格证据和审计回放”的控制面。对象文件仍在 CDE/ObjectStore,向量库仍只是索引,但所有可追责状态必须回到 PostgreSQL。

数据库分域必须按事实来源拆清楚:

| 分域 | 真源表 / 关键表 | 解决的问题 | 不能替代它的东西 |
|---|---|---|---|
| 核心对象域 | `tenants`, `projects`, `modules`, `business_objects`, `object_versions` | 租户、项目、模块、业务对象、对象版本和状态。 | 前端状态、Excel 行、聊天记录。 |
| CDE 文件域 | `module_files`, `file_versions`, `object_store_objects`, `archive_packages` | 源文件、checksum、版本、派生物、归档包。 | PDF 截图、浏览器缓存、下载副本。 |
| 元数据合同域 | `metadata_objects`, `metadata_fields`, `schema_versions`, `field_dictionary`, `metadata_lineage_edges` | 对象类型、字段、单位、必填、标准来源、SJG157/bSDD、Schema 版本和对象血缘。 | Prompt 里的字段说明、前端临时列。 |
| OpenBIM 模型域 | `openbim.ifc_models`, `model_elements`, `element_types`, `property_sets`, `drawing_views`, `ids_specs`, `validation_results`, `bcf_topics` | IFC GUID、构件实例、类型、属性集、图纸视图、IDS、Validate、BCF 模型协同。 | GLB/截图/渲染缓存。 |
| BOM 与计量域 | `bom_documents`, `bom_versions`, `bom_lines`, `quantity_takeoffs`, `material_takeoffs`, `boq_items` | QTO 按模计量、MTO 材料提量、构件 BOM、材料 BOM、BOQ 和成本版本。 | 一个 Excel 表或模型对象本身。 |
| 采购制造施工域 | `quote_lines`, `material_requisitions`, `purchase_requests`, `fabrication_parts`, `work_orders`, `installation_tasks`, `acceptance_records` | 报价、请购、供应商、下料、加工、质检、发料、安装和验收。 | BOM 草稿、聊天确认。 |
| 工作流调度域 | `workflow_runs`, `workflow_tasks`, `approval_tasks`, `operation_queue`, `resource_locks`, `schedule_windows` | 协同状态、审批任务、Worker/Agent 排队、CPU/GPU/NAS 限流。 | Agent 自己抢资源或直接改状态。 |
| AI 运行域 | `agent_profiles`, `agent_runs`, `agent_run_steps`, `agent_tool_calls`, `agent_artifacts`, `model_routes` | 200 个 Agent 的岗位身份、输入 hash、工具调用、模型路由、输出 hash 和审批状态。 | 常驻 200 个 GPU 进程。 |
| 证据审计域 | `price_evidence`, `supplier_quotes`, `purchase_lock_records`, `audit_events`, `event_outbox`, `capacity_snapshots` | 价格锁价、供应商报价、审批、运维、容量扩容和审计回放。 | 没有 SKU/截图/时间/发票条件的口头价格。 |

QTO/MTO/BOM/BOQ/PBOM/MBOM 的数据边界:

| 业务对象 | 数据来源 | 最小字段 | 下游使用 | 发布门禁 |
|---|---|---|---|---|
| QTO 按模计量 | `model_elements`, `drawing_views`, `measure_rules` | `model_element_id`, `ifc_guid`, `measure_rule_id`, `unit`, `quantity`, `rule_version` | BOQ、进度、验收、变更影响 | 必须有模型实例、图纸视图、计量规则和审批状态。 |
| MTO 材料提量 | QTO、材料属性、材料库、损耗规则 | `material_spec_id`, `grade`, `length`, `weight`, `waste_factor`, `batch_rule` | 采购、库存、下料、供应商询价 | 必须有材质、规格、重量/数量、损耗和规则版本。 |
| 构件 BOM | 构件实例、类型定义、属性集、图纸、分类字典 | `bom_version_id`, `bom_line_id`, `model_element_id`, `category_code`, `component_name`, `quantity` | 计量、采购、制造、施工 | 必须回跳源对象、SJG157/bSDD、图纸和审批记录。 |
| BOQ/成本 | QTO、BOM、价格证据、税费规则 | `boq_item_id`, `source_bom_line_id`, `unit_price`, `cost_code`, `price_evidence_id` | 商务报价、预算、变更、财务 | 没有价格证据不得生成正式报价。 |
| PBOM 采购 BOM | MTO、库存、供应商、交期、采购规则 | `purchase_request_id`, `supplier_id`, `sku`, `lead_time`, `locked_price` | 请购、比价、采购审批 | 供应商、SKU、截图、发票和质保条款未锁定时只能待确认。 |
| MBOM 制造 BOM | 构件 BOM、MTO、工艺路线、加工图 | `fabrication_part_id`, `process_route_id`, `work_order_id`, `qc_rule_id` | 下料、焊接、涂装、防火、防腐、质检、包装 | 加工图、工艺、质检规则和责任人未确认不得排产。 |

| 控制面 | 核心表 | 由谁写入 | 由谁消费 | 不允许 |
|---|---|---|---|---|
| 元数据注册 | `metadata_objects`, `metadata_fields`, `schema_versions`, `field_dictionary` | Gateway、Settings、SchemaValidator | 前端表格、API、Agent、报表 | 前端或 Agent 自己发明字段并直接入库。 |
| 工作流运行 | `workflow_templates`, `workflow_runs`, `workflow_tasks`, `approval_tasks` | Workflow Engine、Gateway | 16 模块、个人待办、审批面板 | Worker 绕过 Workflow 直接改业务状态。 |
| 操作队列 | `operation_queue`, `operation_attempts`, `resource_locks`, `schedule_windows` | `srv-05` 调度器 | Worker、Agent Orchestrator、监控 | 200 个 Agent 同时抢数据库、GPU 或 NAS。 |
| Agent 注册与运行 | `agent_profiles`, `agent_runs`, `agent_run_steps`, `agent_tool_calls`, `agent_artifacts` | AI Center、Agent Orchestrator | 审计、成本、质量评估、工作台 AI 面板 | Agent 输出没有输入 hash、工具记录和审批状态。 |
| 价格证据 | `price_books`, `price_snapshots`, `price_evidence`, `supplier_quotes`, `purchase_lock_records` | 造价、采购、财务、PriceSourcingAgent | BOQ、报价、采购、审计 | 没有 SKU、店铺、截图、时间和发票条件就写成正式价格。 |
| OpenBIM 证据 | `openbim.ifc_models`, `openbim.ids_specs`, `openbim.validation_results`, `openbim.bcf_topics`, `bsdd_terms`, `sjg157_mappings` | BIM/IFC Worker、Validate Worker、专业复核 | 发布门禁、归档、review | 只有模型文件,没有 IDS/Validate/BCF/审批证据就声明 ready；bSDD 国际语义 URI 与 SJG157 本地编码可以双挂接。 |
| 容量与健康 | `runtime_health_checks`, `capacity_snapshots`, `db_slow_queries`, `backup_runs`, `restore_drills` | 运维 Worker、Audit Agent | 管理层、运维、采购决策 | 凭感觉决定 512GB、696X 或 GPU 扩容。 |

### 2.6 元数据模型

元数据不是页面字段说明,而是跨模块对象、字段、版本、来源、责任和规则的机器可读合同。所有表单、导入、Agent 输出、API Schema、审批规则和报表都必须引用同一套元数据。

元数据合同分三层:

| 层 | 作用 | 典型字段 | 验收 |
|---|---|---|---|
| 对象层 | 定义业务对象类型和实例身份。 | `object_type`, `schema_ref`, `owner_role`, `state`, `version_no` | 同一对象只有一个权威 ID,跨模块只引用 ID 和版本。 |
| 字段层 | 定义字段、单位、来源、必填、隐私和校验规则。 | `field_key`, `data_type`, `unit`, `required`, `standard_ref`, `validation_rule_id` | 前端、导入模板、API、Agent 输出和报表引用同一字段合同。 |
| 血缘层 | 记录对象之间的生成、引用、审批和派生关系。 | `from_object_id`, `to_object_id`, `relation_type`, `agent_run_id`, `audit_event_id` | 任一 BOM/BOQ/报价/采购/工单可回跳到源文件、模型对象、Schema 和审计事件。 |

工程对象的最小血缘链:

```text
customer_requirement
  -> source_file / meeting_record / contract_clause
  -> model_element / ifc_guid / element_type / property_set
  -> drawing_view / detail_ref
  -> quantity_takeoff / material_takeoff
  -> bom_line / boq_item / quote_line
  -> purchase_request / fabrication_part / work_order
  -> installation_task / acceptance_record
  -> archive_package / audit_event
```

| 表 | 主键 | 关键字段 | 用途 |
|---|---|---|---|
| `metadata_objects` | `metadata_object_id` | `tenant_id`, `module_id`, `object_type`, `schema_ref`, `owner_role`, `state`, `version_no` | 注册业务对象,例如 `bom_line`, `ifc_model`, `quote_line`, `agent_run`。 |
| `metadata_fields` | `metadata_field_id` | `metadata_object_id`, `field_key`, `data_type`, `unit`, `required`, `pii_level`, `standard_ref`, `validation_rule_id` | 注册字段定义、单位、必填、隐私等级和校验规则。 |
| `schema_versions` | `schema_version_id` | `schema_ref`, `json_schema`, `migration_ref`, `effective_from`, `deprecated_at` | 绑定 JSON Schema、数据库迁移和 API 版本。 |
| `metadata_lineage_edges` | `lineage_edge_id` | `from_object_type`, `from_object_id`, `to_object_type`, `to_object_id`, `relation_type`, `agent_run_id`, `audit_event_id` | 记录需求、文件、模型、图纸、BOM、BOQ、报价、采购、工单、验收和档案之间的回跳链。 |
| `field_dictionary` | `dictionary_entry_id` | `term`, `alias`, `module_id`, `standard_source`, `sjg157_code`, `bsdd_uri` | 管理术语、别名、SJG157 本地编码和 bSDD URI 双挂接。 |
| `data_quality_issues` | `issue_id` | `object_type`, `object_id`, `field_key`, `severity`, `rule_ref`, `owner_role`, `status` | 字段缺失、冲突、越权、重复主数据和规则缺口的待办。 |
...
CREATE TABLE metadata_objects (
    ...
    metadata JSONB NOT NULL DEFAULT '{}'::jsonb,
    created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    UNIQUE(tenant_id, module_id, object_type, version_no)
);

CREATE TABLE metadata_fields (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    metadata_object_id UUID NOT NULL REFERENCES metadata_objects(id) ON DELETE CASCADE,
    ...
    metadata JSONB NOT NULL DEFAULT '{}'::jsonb,
    UNIQUE(metadata_object_id, field_key)
);

CREATE TABLE metadata_lineage_edges (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    tenant_id UUID NOT NULL,
...
用户问的“协同管理调度数据库智能体”不是一个单独聊天 Agent,而是一组服务于 `settings_center`、`ai_center`、`planning_management`、`digital_archive` 和全模块工作台的治理 Agent。它们的运行位置是:配置和状态在 `srv-01` PostgreSQL,排队和限流在 `srv-05`,通用巡检任务在 `srv-04`,BIM/模型相关任务派到 `srv-03`,文件证据写 `srv-02`,审计证据写 `srv-06`。

| Agent | 服务对象 | 读取 | 写入 | 禁止事项 |
|---|---|---|---|---|
| `CollaborationCoordinatorAgent` | 项目经理、部门负责人、个人中心 | `workflow_runs`, `approval_tasks`, `operation_queue`, `project_members` | 待办建议、协同阻塞原因、升级建议 | 不得自动替负责人审批或改责任矩阵。 |
| `SchedulingOrchestratorAgent` | 调度器、Worker、Agent 队列 | `operation_queue`, `resource_locks`, `schedule_windows`, `capacity_snapshots` | 排队优先级建议、批处理窗口、资源冲突待办 | 不得绕过限流把 200 个 Agent 同时打到数据库/GPU/NAS。 |
| `DatabaseStewardAgent` | DBA、系统管理员 | `pg_stat`, `db_slow_queries`, `schema_versions`, `backup_runs` | 慢查询建议、索引建议、迁移风险、备份异常 | 不得直接执行破坏性 DDL/DML,必须生成审批任务。 |
| `MetadataGovernanceAgent` | 设置治理、标准库、AI 中心 | `metadata_objects`, `metadata_fields`, `field_dictionary`, `data_quality_issues` | 字段冲突、缺失字典、Schema 变更建议 | 不得跳过 SchemaValidator 发布字段。 |
| `PriceEvidenceAgent` | 造价、采购、财务 | `price_books`, `price_snapshots`, `price_evidence`, `supplier_quotes` | 价格缺证据项、过期价格、采购锁价待办 | 不得在缺少京东/供应商截图和 SKU 时生成正式采购价。 |
| `OpenBimEvidenceAgent` | BIM 负责人、档案、审计 | `ifc_models`, `ids_specs`, `validation_results`, `bcf_topics`, `approval_tasks` | OpenBIM 证据缺口、review 阻断项 | 不得把未认证/未一致性测试结果写成官方通过。 |
...
CREATE TABLE operation_queue (
    ...
    required_resource JSONB NOT NULL DEFAULT '{}'::jsonb,
...
CREATE TABLE resource_locks (
    ...
    operation_id UUID REFERENCES operation_queue(id),
...
CREATE TABLE capacity_snapshots (
    ...
    mem_used_gb NUMERIC NOT NULL,
    mem_total_gb NUMERIC NOT NULL,
    gpu_mem_used_gb NUMERIC,
    gpu_mem_total_gb NUMERIC,
...
当前文档中所有硬件、网络、安全、UPS、机柜、线材等价格都必须进入“价格证据”表。没有实时 SKU、店铺、截图、时间、发票类型和质保条款的价格,只能标记为 `budget_placeholder` 或 `pending_jd_lock`,不能写成最终采购价。当前环境自动抓取京东搜索页无法稳定取得实时商品价,因此文档不得用无法复核的假精确价格替代采购证据。

| 价格状态 | 含义 | 可用于 |
|---|---|---|
| `user_input` | 用户在会议或聊天中明确给出的单价 | 预算测算,但采购前仍需截图锁价。 |
| `pending_jd_lock` | 需要按京东自营/品牌旗舰店实时 SKU 截图 | 待办,不能作为采购单价格。 |
| `supplier_quote` | 供应商正式报价单 | 采购比价,需财务/采购复核。 |
| `locked` | 已保存 SKU、店铺、截图、时间、发票、质保和审批 | 可进入采购审批。 |
| `expired` | 超过有效期或 SKU 失效 | 不得继续用于报价或采购。 |
...
CREATE TABLE price_evidence (
    ...
    price_status TEXT NOT NULL DEFAULT 'pending_jd_lock',
    captured_at TIMESTAMPTZ,
    evidence_file_id UUID REFERENCES module_files(id),
...
### 2.9 数据库容量与 512GB 内存判断

当前实际需求不是所有服务器直接上 512GB。L2 内部生产力和 L3 受控试点的数据库基线是 `srv-01` 64GB 起步,优先升 256GB；512GB 只在可观测指标证明内存成为瓶颈时进入采购。决策依据写入 `capacity_snapshots`、`db_slow_queries`、`backup_runs` 和 `operation_queue`,不能凭感觉扩容。

| 节点 | 建议内存 | 触发 512GB 的业务场景 | 判断指标 |
|---|---:|---|---|
| `srv-01-cde-api-db` | 64GB -> 256GB 优先 | PostgreSQL 同时承载业务事务、元数据、pgvector/Search 辅助索引、工作流队列、Agent 运行索引,且 100/1000 用户试点下热数据和连接池持续吃内存 | 内存常驻 >75%、慢查询增加、缓存命中下降、队列延迟影响业务。 |
| `srv-03-bim-ifc-derivative` | 64GB -> 256GB 优先 | 大型 IFC/STEP/OpenUSD/点云/多模型 federation、模型导图、模型导 BOM、几何缓存和 GPU feeding 同时运行 | 单任务内存峰值 >180GB、GPU 空等 CPU/RAM、派生任务频繁 OOM。 |
| `srv-02-nas-cde-backup` | 64GB -> 128/256GB | ZFS ARC、对象校验、快照、备份恢复和大量 CDE 文件并发读取成为瓶颈 | ARC 命中率低、备份窗口过长、恢复校验拖慢业务。 |
| `srv-05-app-api-queue` | 64GB -> 128/256GB | 200 Agent 目录规模下多项目排队、缓存、Search 辅助服务和 API 并发上升 | 队列延迟、缓存淘汰、连接池等待、API P95 延迟升高。 |
| `srv-04`/`srv-06` | 64GB/128GB 通常足够 | 大批量 Office/PDF/CAD 解析或日志审计检索明显吃内存 | Worker OOM、日志查询拖慢、CI 测试库占用过高。 |

结论:一期先把 `srv-01` 和 `srv-03` 作为 256GB 优先节点,`srv-02` 视 ZFS/备份指标升 128GB 或 256GB。512GB 是 P1/P2 扩容项,优先候选是 `srv-03` 大模型/大文件派生节点和 `srv-01` 数据库/元数据/队列节点,不是六台机器的默认配置。若仍使用 64GB RDIMM,单台 64GB -> 512GB 需要 8 根 64GB,即在基线外追加 7 根；256GB -> 512GB 追加 4 根。具体单价必须进入 `price_evidence`,按京东或供应商实时锁价后再计算。
...
| `ai_center` | `SchedulingOrchestratorAgent` | Agent 队列、Worker 状态、GPU/CPU/NAS 资源 | 调度优先级、限流和批处理窗口建议 |
| `settings_center` | `PermissionReviewAgent` | 用户、岗位、审批矩阵 | 权限建议 |
| `settings_center` | `MetadataGovernanceAgent` | 元数据对象、字段、Schema、数据字典 | 字段冲突、Schema 变更和数据质量待办 |
| `settings_center` | `DatabaseStewardAgent` | 慢查询、连接池、备份、迁移、容量快照 | 索引、分区、备份和扩容建议 |
| `quantity_costing` | `PriceEvidenceAgent` | 京东/供应商价格证据、SKU、截图、发票和质保 | 价格锁定状态、过期提醒、采购证据缺口 |
...
本节覆盖 ArchIToken 一期落地所需的 6 台 CPU 服务器、NAS、网络、安全、UPS、备份和 BIM GPU 专项另购。当前推荐基线是 2 台 Xeon 676X + 4 台 Xeon 658X:676X 用于 CDE/API/数据库关键节点和 BIM/IFC/STEP/STL/USDZ/OpenUSD 模型派生节点；658X 用于 NAS、CI/通用 Worker、应用/API/队列、JumpServer/日志/审计。根据实际负载,`srv-01` 数据库/API、`srv-03` BIM/模型派生或其它瓶颈节点可选 Xeon 696X,按用户确认单价 ¥50,000/颗列入可选升级。每台服务器基线至少 64GB ECC RDIMM,并把硬盘线/转接线、散热膏、风扇、扎带、螺丝和理线等小配件按 ¥1,000/台列入核心物料；`srv-01`、`srv-02`、`srv-03` 等关键节点可按实际负载增配到 256GB。研发、设计人员已有日常台式机,本轮不采购开发/设计整机。BIM GPU 可二选一:方案 A 为 2 张 NVIDIA RTX PRO 6000D Blackwell Server Edition 84GB PCIe,单价 ¥68,000/张,小计 ¥136,000；方案 B 为 4 张 RTX 5090 32GB 双宽涡轮,单价 ¥36,000/张,小计 ¥144,000。
...
| BIM GPU 加速卡 | 是,专项另购 | 方案 A:2 张 RTX PRO 6000D 84GB,小计 ¥136,000,总显存 168GB,约 1200W GPU 功耗；方案 B:4 张 RTX 5090 32GB 双宽涡轮,小计 ¥144,000,总显存 128GB,按官方 RTX 5090 TGP 575W 估算约 2300W GPU 功耗。用于 BIM/IFC/STEP/STL/USDZ/OpenUSD 派生、模型导图、构件 BOM、局部视觉/几何推理和渲染加速实验。 |
| BIM/GPU 服务器整机 | 是,按 `srv-03` 试点节点采购 | `srv-03` 使用 Xeon 676X、长城黑匣子 15 机箱和长城黑匣子 3200W ATX3.1 电源承接 BIM GPU。2 张 RTX PRO 6000D 是优先稳妥路线；4 张 RTX 5090 只有在机箱、PCIe、供电线、风道和驱动验收通过后才能进入同一节点,否则必须另立 GPU 合规服务器预算。 |
...
|---|---|---:|---|
| Intel Xeon 696X | 可选高算力节点:数据库/API 高并发、BIM/IFC/STEP/STL/USDZ/OpenUSD 模型派生、几何缓存、本地推理和重型 Worker | 按需选配 | 用户确认单价 ¥50,000/颗。Intel 官方规格为 64 核 / 128 线程、336MB Cache、2.40GHz 基础频率、4.80GHz 最大睿频、350W 基础功耗、420W 最大睿频功耗、8 通道 DDR5/MRDIMM、PCIe 5.0 128 lanes。功耗和散热压力显著高于 676X/658X,必须复核主板 BIOS/QVL、散热器、机箱风道和电源余量。 |
...
| RTX 5090 32GB 双宽涡轮 | BIM/IFC/STEP/STL/USDZ/OpenUSD 派生、局部推理、并行渲染和多任务队列 | 备选,专项另购 | 4 张,单价 ¥36,000/张,显卡小计 ¥144,000。优点是并行卡数更多；约束是总显存 128GB、功耗和散热压力更高、消费级 GeForce 路线必须单独验收驱动、质保、持续负载和机箱兼容。 |
...
价格口径: CPU、主板、内存、机械盘、PM9A3、英睿达/美光 T710 Pro 4TB、长城电源、NAS 机箱、服务器机箱/电源、每台小配件和 GPU 使用用户/会议确认单价做预算测算；交换机、防火墙、UPS、机柜、AP、布线等不再写假精确价格,统一进入 `price_evidence` 表,状态为 `pending_jd_lock`,下单前必须逐项用京东自营或品牌旗舰店 SKU、截图、发票和质保条款锁价。

| 类别 | 设备/物料 | 数量 | 单价/口径 | 小计 | 用途 |
|---|---|---:|---:|---:|---|
| CPU 服务器 | `srv-01-cde-api-db` | 1 台 | 当前物料明细 | ¥58,450 | CDE 元数据、API Gateway、PostgreSQL、Outbox、ModelRouter、权限、审计事件。 |
| CPU 服务器 | `srv-02-nas-cde-backup` | 1 台 | 当前物料明细 | ¥83,200 | CDE 源文件、对象存储、IFC/STL/STEP/USDZ/OpenUSD、图纸、BOM、归档包、ZFS 快照、离线归档；NAS 使用 Xeon 658X、热插拔 NAS 机箱和 2 个长城服务器电源。 |
| CPU 服务器 | `srv-03-bim-ifc-derivative` | 1 台 | 当前物料明细 | ¥58,650 | BIM/IFC/STEP/STL/USDZ/OpenUSD 模型派生、模型导图、模型导出构件 BOM、Validate Worker；使用长城黑匣子 15 机箱和长城黑匣子 3200W 电源。 |
| CPU 服务器 | `srv-04-ci-general-worker` | 1 台 | 当前物料明细 | ¥46,800 | Office/PDF/CAD 解析、批量导入、Schema 校验、CI、测试数据库、任务执行。 |
| CPU 服务器 | `srv-05-app-api-queue` | 1 台 | 当前物料明细 | ¥46,800 | 模块应用服务、内部 API、任务队列、缓存、Search/Vector 辅助服务、试点入口。 |
| CPU 服务器 | `srv-06-jump-log-audit` | 1 台 | 当前物料明细 | ¥46,800 | 开源 JumpServer、运维入口、日志、监控、备份校验、只读审计、审计报表。 |
| CPU 核心物料合计 | 2 x 676X + 4 x 658X 六节点 | - | - | **¥340,700** | 每台至少 64GB ECC RDIMM；已含每台 ¥1,000 线材/散热/小配件；CPU 包本身不是 HA 生产机房口径。 |
| CPU 可选升级 | 676X 节点升级为 696X | 每台按需 | ¥50,000 - ¥25,350 | +¥24,650/台 | 优先用于 `srv-01` 数据库/API 或 `srv-03` BIM/模型派生；若两台 676X 均升 696X,CPU 核心物料从 ¥340,700 变为 ¥390,000。 |
| CPU 可选升级 | 658X 节点升级为 696X | 每台按需 | ¥50,000 - ¥17,500 | +¥32,500/台 | 只在 `srv-04` Worker、`srv-05` 队列或其它 658X 节点出现明确 CPU 瓶颈时采用；NAS 不优先升 696X。 |
| 网络/安全/供电 | 防火墙、10GbE 核心交换、2.5GbE PoE、AP、UPS、机柜和布线 | 1 套 | `pending_jd_lock` | 待锁价 | 双运营商入口、内网分区、无线、VPN/MFA、等保基础日志和断电保护；没有京东 SKU/截图前不计入已确认物料。 |
| 离线备份盘 | Toshiba MG11 22TB | 2 块 | ¥7,400 | ¥14,800 | 企业盘离线轮换备份,不常挂载,防误删和勒索软件。 |
| CPU/存储已确认物料合计 | 六节点 CPU 核心物料 + 离线备份 | - | - | **¥355,500** | 不含网络/安全/UPS/机柜/布线待锁价项,不含公网专线月租,不含商业软件授权,不含如需追加的 GPU 合规服务器。 |
| BIM GPU 方案 A | NVIDIA RTX PRO 6000D Blackwell Server Edition 84GB PCIe | 2 张 | ¥68,000 | **¥136,000** | 总显存 168GB,约 1200W GPU 功耗；更适合服务器版稳妥采购、长时派生和大显存任务。不能替代 OpenBIM 校验和认证证据。 |
| BIM GPU 方案 B | RTX 5090 32GB 双宽涡轮 | 4 张 | ¥36,000 | **¥144,000** | 总显存 128GB,按官方 RTX 5090 TGP 575W 估算约 2300W GPU 功耗；必须复核四卡机箱、PCIe、供电、风道、驱动、质保和持续负载温度。 |
| 已确认物料 + GPU 方案 A | 六节点 CPU/存储已确认物料 + 2 张 RTX PRO 6000D | - | - | **¥491,500** | 当前服务器版稳妥口径；网络/安全/UPS/机柜/布线仍待京东锁价。若 GPU 上架验收发现机箱、PCIe、供电、风道、驱动或温度监控不满足,必须单独追加 GPU 合规服务器或配件预算。 |
| 已确认物料 + GPU 方案 B | 六节点 CPU/存储已确认物料 + 4 张 RTX 5090 | - | - | **¥499,500** | 并行卡数更多,但供电/散热/质保风险更高；网络/安全/UPS/机柜/布线仍待京东锁价,不得默认替代服务器版方案。 |
...
| 线材/散热/小配件 | 1 套 | ¥1,000 | ¥1,000 | 硬盘线/转接线、散热膏、风扇、扎带、螺丝、理线和其它装机小配件；下单按实际机箱、主板、硬盘和电源接口复核。 |
| 小计 | - | - | **¥58,450** | 作为 CDE/API/数据库关键节点,不是高可用数据库集群。 |
...
| 银昕 CS383 热插拔 NAS 机箱 | 1 台 | ¥3,000 | ¥3,000 | 8 盘位热插拔 NAS 机箱；8 x 3.5/2.5 英寸 SAS-12G/SATA-6G 热插拔盘托,支持 E-ATX/SSI-EEB,一期先装 4 块 MG11。 |
| 线材/散热/小配件 | 1 套 | ¥1,000 | ¥1,000 | 硬盘线/SlimSAS 或 SATA 转接线、散热膏、风扇、扎带、螺丝、理线和其它装机小配件；NAS 下单必须复核背板线缆和硬盘风道。 |
| 小计 | - | - | **¥83,200** | 按本轮已确认物料计算；已包含每台 ¥1,000 小配件,但若实际需要 HBA、特殊转接线或额外风扇墙,必须另列实价。 |
...
| 物料 | 最低规格/用途 | 数量 | 价格状态 | 下单前必须补齐 |
|---|---|---:|---|---|
| 企业防火墙/路由 | 支持 1Gbps/10Gbps 入口、VPN/MFA、日志外送、策略路由 | 1 台 | `pending_jd_lock` | 京东自营/品牌旗舰店 SKU、实时单价截图、发票类型、授权/质保、吞吐和日志能力。 |
| 10GbE 管理型核心交换 | VLAN、ACL、LACP、SNMP/日志,连接服务器、NAS、防火墙 | 1 台 | `pending_jd_lock` | 端口数量、10GbE 形态、光模块/DAC 兼容、管理功能、质保和截图。 |
| 2.5GbE PoE 接入交换 | 桌面重点位和 Wi-Fi 7 AP 供电 | 1 台 | `pending_jd_lock` | PoE 总功率、端口数量、上联速率、散热和截图。 |
| Wi-Fi 7 AP | 办公区、会议室、访客区覆盖 | 4 台暂列 | `pending_jd_lock` | AP 型号、PoE 需求、控制器/云管方式、覆盖图和截图。 |
| 在线式 UPS | 服务器、NAS、网络设备断电保护和自动关机 | 1 套 | `pending_jd_lock` | kVA/kW、续航、SNMP/USB 管理、电池保修、安装和截图。 |
| 机柜/PDU/环境监控 | 机柜、PDU、温湿度、门磁或基础监控 | 1 套 | `pending_jd_lock` | U 数、深度、承重、PDU 路数、温湿度/门磁、安装和截图。 |
| 光模块/DAC/六类线/理线 | 服务器、交换、防火墙、AP 接入布线 | 1 批 | `pending_jd_lock` | 线缆类型、长度、数量、品牌、光模块兼容清单和截图。 |

以上项目不再合并成固定小计。采购前由 `PriceEvidenceAgent` 生成 `price_evidence` 记录,只有状态达到 `locked` 后才能进入采购审批和预算合计。
...
| 场景 | 是否允许 | 说明 |
|---|---:|---|
| 100 用户内测/内部研发 | 可以临时合并,但不作为当前推荐采购口径 | 当前推荐已经拆成 6 台轻量试点节点；单机合并只适合极短期开发验证,不进入本轮采购总表。 |
| 1000 用户商业试点 | 不建议合并 | 当前按 `srv-01` CDE/API/数据库、`srv-02` NAS、`srv-03` BIM 派生、`srv-04` Worker、`srv-05` 应用队列、`srv-06` 运维审计拆分故障域；公网入口要双运营商。 |
...
| 负载 | 一期承载位置 | 说明 |
|---|---|---|
| 16 模块工作台 | `srv-05` + `srv-01` | `srv-05` 承载 Next.js/React 前端、模块应用服务、API 队列和缓存；`srv-01` 承载 Gateway、数据库、RBAC 和审计事件。 |
...
| Agent 编排与岗位任务队列 | `srv-01` Agent Registry + `srv-05` 调度队列 + `srv-04` 通用 Worker + `srv-03` BIM/GPU Worker | 200 个 Agent 是注册项、策略、队列任务和岗位能力目录,不是 200 个常驻 GPU 进程；文本/结构化 Agent 主要跑在 `srv-04` 和外部/本地模型路由,只有 BIM/开放格式、视觉、渲染和本地大模型任务进入 `srv-03` GPU 队列。 |
| JumpServer 堡垒机 | `srv-06` | 会话录像和命令审计写 `srv-02`。 |
| 日志审计 | `srv-06` 热日志 + `srv-02` 冷归档 | 生产日志、Agent 调用、运维命令、审批记录必须可追溯。 |
| BIM/CAD 派生 worker | `srv-03`/`srv-04` CPU worker + BIM GPU 专项卡 | CPU worker 负责解析、队列和写回 CDE；GPU 可选方案 A:2 张 RTX PRO 6000D 84GB 服务器版,或方案 B:4 张 RTX 5090 32GB 双宽涡轮。正式图纸、BOM、报价仍需规则校验和审批。 |

#### 5.25.10 分阶段扩容

| 阶段 | 触发条件 | 新增硬件 | 预算口径 |
|---|---|---|---|
| 一期 L2+/L3 六节点试点包 | 30 人内部生产力和 100/1000 用户试点需要拆分 CDE、NAS、Worker、应用、审计故障域 | 2 台 Xeon 676X + 4 台 Xeon 658X CPU 服务器 + 离线备份；网络安全和 UPS 待锁价 | CPU 核心物料 ¥340,700；加离线备份后已确认物料 ¥355,500；网络/安全/UPS/机柜/布线不再用假总价 |
| BIM GPU 专项另购 A | BIM/IFC/STEP/STL/USDZ/OpenUSD 派生、模型导图、构件 BOM、局部推理和渲染加速需要本地大显存卡 | NVIDIA RTX PRO 6000D Blackwell Server Edition 84GB PCIe x2 | 显卡小计 ¥136,000；已确认物料 + GPU A 为 ¥491,500；网络安全另行京东锁价 |
| BIM GPU 专项另购 B | 更看重并行卡数和预算接近时的多任务队列 | RTX 5090 32GB 双宽涡轮 x4 | 显卡小计 ¥144,000；已确认物料 + GPU B 为 ¥499,500；网络安全另行京东锁价；四卡供电、散热和质保必须单独验收 |
...
|---|---:|---|---|
| Intel Xeon 696X | ¥50,000 | 用户确认可选单价 | 只作为关键节点可选升级；截图 SKU、店铺、发票类型、质保条款；复核 64 核/128 线程、336MB、350W/420W、DDR5/MRDIMM、PCIe 5.0 128 lanes、FCLGA4710、主板 BIOS/QVL、散热器 TDP、机箱风道和电源余量。 |
...
| RTX 5090 32GB 双宽涡轮 | ¥36,000 | 用户确认单价 | 作为 GPU 方案 B 采购 4 张；复核双宽涡轮 SKU、32GB 显存、575W TGP、600W 供电线、四卡间距、散热、持续负载、驱动、质保和是否适合服务器长期运行。 |
| 技嘉 MW94-RP0 主板 | ¥8,000 | 用户确认单价 | 复核 Xeon 676X BIOS 支持、LGA4710-2、W890、8 通道 RDIMM/MRDIMM、2 x 10GbE、SlimSAS/SATA、M.2、CS383 安装兼容。 |
| NAS 机箱 | ¥3,000 | 用户确认单价 | `srv-02` NAS 使用热插拔 NAS 机箱；下单前复核 8 盘位、背板线缆、风扇、硬盘散热、主板尺寸和双长城服务器电源兼容。 |
| BIM 机箱 | ¥2,000 | 用户确认单价 | `srv-03` BIM/IFC/开放格式模型派生节点使用长城黑匣子 15 机箱；必须复核双 RTX PRO 6000D 或四 RTX 5090 的长度、厚度、导风、固定方式和维护空间。 |
| 通用服务器机箱 | ¥1,000 | 用户确认单价 | 除 NAS 机箱和 BIM 机箱外,`srv-01`/`srv-04`/`srv-05`/`srv-06` 可按服务器机箱 ¥1,000 暂列；下单前复核主板、散热器、盘位、导轨和风道。 |
| BIM 电源 | ¥4,000 | 会议确认单价 | `srv-03` BIM 节点使用长城黑匣子 3200W ATX3.1；必须复核双 RTX PRO 6000D 或四 RTX 5090 供电线、瞬时功耗、输入电压、CQC/质保和机箱安装兼容。 |
| 通用服务器电源 | ¥1,000 | 用户确认单价 | `srv-01`/`srv-04`/`srv-05`/`srv-06` 各 1 个；NAS 使用 2 个长城服务器电源；均需复核机箱兼容、线材、冗余/并联方式、质保和接口实物。 |
| 硬盘线/转接线/散热膏/风扇等小配件 | ¥1,000/台 | 用户确认单价 | 6 台服务器均列入；覆盖硬盘线、转接线、散热膏、风扇、扎带、螺丝、理线和装机耗材。GPU 额外专用供电线、导风罩或转接件若超出该口径,另列实价。 |
...
| 硬件 | 参数 | 本方案采用方式 |
|---|---|---|
| Intel Xeon 696X | Intel 官方规格列出 64 核 / 128 线程、336MB Cache、2.40GHz 基础频率、4.80GHz 最大睿频、350W 基础功耗、420W 最大睿频功耗、DDR5-6400/MRDIMM-8000、8 通道内存、最大 4TB、ECC、PCIe 5.0、128 lanes、FCLGA4710、Q1 2026 发布。来源: <https://www.intel.cn/content/www/cn/zh/products/sku/243860/intel-xeon-696x-processor-336m-cache-2-40-ghz/specifications.html>；订购页列出 Tray 订购号 `PK8072006491900`、Boxed 订购号 `BX80720696X`、China boxed 订购号 `BXC80720696X`,建议客户价格 $6159-$6169。来源: <https://www.intel.cn/content/www/cn/zh/products/sku/243860/intel-xeon-696x-processor-336m-cache-2-40-ghz/ordering.html> | 可作为 `srv-01` 数据库/API 和 `srv-03` BIM/开放格式模型派生的按需升级 CPU。由于 420W 最大睿频功耗高于 676X 的 330W,采购前必须确认 MW94-RP0 BIOS/QVL、散热器、机箱风道、电源余量和持续负载温度。 |
...
| NVIDIA RTX PRO 6000D Blackwell Server Edition 84GB PCIe | HPE QuickSpecs 列出 NVIDIA RTX PRO 6000D Blackwell Server Edition 84GB PCIe Accelerator for HPE,型号 S6W21C；规格包括 84GB GDDR7、1398GB/s Memory Bandwidth、PCIe Gen5 x16、最高 2 MIG、600W,销售区域标注为 China/Hong Kong/Macau。来源: <https://www.hpe.com/psnow/downloadDoc/NVIDIA%20Accelerators%20for%20HPE%20QuickSpecs-c04123180.pdf?contentDisposition=attachment&deepLink=&form=false&hf=regular&id=c04123180.pdf&isFutureVersion=true&isLinearized=false&originalObjectName=&prelaunchSection=&preview=false&print=&r=&section=&softrollSection=&ver=78> | GPU 方案 A 专项另购 2 张。验收重点不是只看显存,而是服务器平台是否支持双卡 1200W GPU 峰值供电、持续风道、PCIe 带宽、驱动、容器运行时、温度监控和故障告警。 |
| NVIDIA GeForce RTX 5090 32GB | NVIDIA 官方 RTX 5090 规格页列出 32GB GDDR7、PCIe Gen 5、Total Graphics Power 575W、Required System Power 1000W、600W 供电线。来源: <https://www.nvidia.com/en-us/geforce/graphics-cards/50-series/rtx-5090/> | GPU 方案 B 按 4 张双宽涡轮采购。它是消费级 GeForce 路线,四卡总 TGP 约 2300W；必须单独验收服务器机箱四卡间距、PCIe lane/slot、供电线、进出风、驱动、质保和持续负载温度,不得默认等同服务器版 GPU。 |
...
| 长城黑匣子 3200W ATX3.1 电源 | 铂金+CQC-IV、ATX 3.1、输入电压 198-264VAC、14cm 风扇、CQC 认证、I/O 开关、工作海拔 ≤5000m、典型载效率 >94%、全模组、ATX 24PIN 线长 600mm、CPU 线长 650mm/接口 2 个、PCI-E 6+2 线长 600mm/接口 8 个、PCIe 5.1 线长 675mm/接口 4 个、SATA 接口 15 个、L4P 接口 6 个、S4P 无、尺寸 175x150x86mm。 | `srv-03` BIM/IFC/开放格式模型派生节点使用该 3200W 电源承接 GPU 方案 A 或方案 B。它仍是 ATX 单电源,不是服务器双冗余热插拔电源；采购验收必须核对 SKU、线材数量、接口实物、CQC/质保、发票、机箱安装兼容、四卡/双卡持续负载温度和故障告警。 |

#### 5.25.13 十页浓缩版对齐: 六节点 CPU 试点包

十页汇报版采用的“一期 6 台 CPU 服务器”不是高可用生产机房口径,而是 L2+/L3 试点轻量包:2 台 Xeon 676X 用于 CDE/API/数据库和 BIM/IFC/开放格式模型派生关键节点,4 台 Xeon 658X 用于 NAS、通用 Worker、应用队列、堡垒机、日志和审计；部分关键节点可按需求选用 Xeon 696X,单价 ¥50,000/颗。每台服务器至少 64GB ECC RDIMM,并列入硬盘线/转接线、散热膏、风扇、扎带、螺丝和理线等小配件 ¥1,000/台；`srv-01`、`srv-02`、`srv-03` 等关键节点可按实际需要升到 256GB。非数据库真源盘、非关键 Worker 和缓存盘可选英睿达/美光 T710 Pro 4TB。BIM 节点使用长城黑匣子 3200W 电源和长城黑匣子 15 机箱；NAS 使用热插拔 NAS 机箱 ¥3,000 和 2 个长城服务器电源；其它非 NAS/非 BIM 节点按服务器机箱 ¥1,000、服务器电源 ¥1,000 暂列。CPU 包本身不含 HA；BIM GPU 专项另购有方案 A:2 张 RTX PRO 6000D 84GB 服务器版,小计 ¥136,000,和方案 B:4 张 RTX 5090 32GB 双宽涡轮,小计 ¥144,000。

| 节点 | 面向类型 | 工作内容 | 硬件配置 | 核心物料小计 | 边界 |
|---|---|---|---|---:|---|
| `srv-01-cde-api-db` | 核心数据与服务入口节点 | CDE 元数据、API Gateway、PostgreSQL、Outbox、ModelRouter、权限、审计事件 | Xeon 676X x1；MW94-RP0；64GB ECC RDIMM；PM9A3 960GB x2；服务器机箱 ¥1,000；服务器电源 ¥1,000；线材/散热/小配件 ¥1,000 | ¥58,450 | 数据库热数据仍需备份到 NAS；不是 HA 主从；数据库、索引和队列压力上来后优先升 256GB。 |
| `srv-02-nas-cde-backup` | NAS、备份、CDE 文件节点 | CDE 源文件、IFC/STL/STEP/USDZ/OpenUSD、图纸、BOM 派生物、数据库备份、离线归档 | Xeon 658X x1；MW94-RP0；64GB ECC RDIMM；PM9A3 960GB x2；MG11 22TB x4；热插拔 NAS 机箱 ¥3,000；长城服务器电源 x2,小计 ¥2,000；线材/散热/小配件 ¥1,000 | ¥83,200 | NAS 用 658X 足够；电源需复核机箱兼容和冗余方式；ZFS ARC 不足时升 256GB。 |
| `srv-03-bim-ifc-derivative` | BIM、IFC、开放格式模型派生节点 | 模型生成草稿、IFC/STL/STEP/USDZ/OpenUSD 导出、模型导出图纸、模型导出构件 BOM、Validate 校验 | Xeon 676X x1；MW94-RP0；64GB ECC RDIMM；T710 Pro 4TB x1；长城黑匣子 15 机箱 ¥2,000；长城黑匣子 3200W 电源 ¥4,000；线材/散热/小配件 ¥1,000 | ¥58,650 | BIM GPU 另购方案 A 或 B；模型派生、几何缓存和本地推理排队后优先升 256GB。 |
| `srv-04-ci-general-worker` | CI、通用解析和批处理节点 | Office/PDF/CAD 解析、批量导入、Schema 校验、CI、测试数据库、任务执行 | Xeon 658X x1；MW94-RP0；64GB ECC RDIMM；T710 Pro 4TB x1；服务器机箱 ¥1,000；服务器电源 ¥1,000；线材/散热/小配件 ¥1,000 | ¥46,800 | Worker 不直接写真源库,结果经 Gateway 入库；高并发批处理后再升 128GB/256GB。 |
| `srv-05-app-api-queue` | 应用、API、任务队列节点 | 模块应用服务、内部 API、任务队列、缓存、Search/Vector 辅助服务、试点入口 | Xeon 658X x1；MW94-RP0；64GB ECC RDIMM；T710 Pro 4TB x1；服务器机箱 ¥1,000；服务器电源 ¥1,000；线材/散热/小配件 ¥1,000 | ¥46,800 | 与数据库、NAS 分离,但不承诺公网生产 SLA；任务队列和缓存扩大后可升 128GB/256GB。 |
| `srv-06-jump-log-audit` | 堡垒机、日志、审计和监控节点 | 开源 JumpServer、运维入口、日志、监控、备份校验、只读审计、审计报表 | Xeon 658X x1；MW94-RP0；64GB ECC RDIMM；T710 Pro 4TB x1；服务器机箱 ¥1,000；服务器电源 ¥1,000；线材/散热/小配件 ¥1,000 | ¥46,800 | 只开堡垒入口和审计出口,不承载业务写流量；通常 64GB/128GB 足够。 |
| CPU 合计 | 2 x 676X + 4 x 658X | 六节点核心物料 | - | **¥340,700** | 已含每台 ¥1,000 小配件；网络、防火墙、UPS、机柜、布线和实施必须京东锁价后另计。 |
| 696X 按需升级 | 676X -> 696X | `srv-01` 或 `srv-03` | 每台 +¥24,650 | 按实际选择 | 两台 676X 都升 696X 时,CPU 核心物料为 **¥390,000**；必须复核 420W CPU 散热、BIOS/QVL、风道和电源余量。 |
| 696X 按需升级 | 658X -> 696X | 瓶颈 Worker/API 节点 | 每台 +¥32,500 | 按实际选择 | 只在 `srv-04`/`srv-05` 等出现明确 CPU 瓶颈时采用；NAS 不优先升 696X。 |
| BIM GPU 方案 A | RTX PRO 6000D 84GB Server Edition | 2 张 GPU 加速卡 | 2 x ¥68,000 | **¥136,000** | 服务器版稳妥路线；不含如需追加的 GPU 合规服务器、导风罩、电源线、商业驱动/软件授权；不能替代 OpenBIM 校验和认证证据。 |
| BIM GPU 方案 B | RTX 5090 32GB 双宽涡轮 | 4 张 GPU 加速卡 | 4 x ¥36,000 | **¥144,000** | 并行卡数更多,但四卡总 TGP 约 2300W；供电、散热、驱动、质保和持续负载温度必须单独验收。 |
| 已确认物料 + GPU A | 六节点 CPU 核心物料 + 离线备份 + 2 张 RTX PRO 6000D | 已确认硬件物料小计 | - | **¥491,500** | 网络/安全/UPS/机柜/布线必须京东锁价后另计。 |
| 已确认物料 + GPU B | 六节点 CPU 核心物料 + 离线备份 + 4 张 RTX 5090 | 已确认硬件物料小计 | - | **¥499,500** | 网络/安全/UPS/机柜/布线必须京东锁价后另计。 |
...
| BIM GPU 专项包 A | BIM/IFC/STEP/STL/USDZ/OpenUSD 派生、模型导图、构件 BOM、局部推理和渲染加速 | 2 张 84GB 服务器版 GPU 可先支撑重型派生实验 | 需要确认目标服务器双卡供电、散热、PCIe、驱动和温度监控 | 作为稳妥服务器版优先方案,但不替代 OpenBIM 校验/认证证据。 |
| BIM GPU 专项包 B | 多卡并行、较多中等显存任务和预算接近时的备选 | 4 张 5090 并行卡数更多,单价已确认 | 消费级路线的供电、散热、质保和长期稳定性风险更高 | 只能作为备选方案,四卡平台验收通过后才采购上架。 |
| GPU/HA 生产包 | 1 万公网生产、HA、多租户 SLA、大规模本地推理 | 支撑重负载和 SLA | 预算超出 40-50 万 CPU 基础包 | 二期/三期单独立项。 |

#### 5.25.14 256GB 内存增配口径

六台服务器基线均按 64GB ECC RDIMM 计入核心物料。部分机器可以按实际负载升到 256GB,但这是可选增配,不混入基线价。按当前 64GB ECC RDIMM 单价 ¥13,500 计算,单台从 64GB 升到 256GB 需要追加 3 根 64GB,即 **+¥40,500/台**。

| 增配对象 | 优先级 | 触发条件 | 增配方式 | 追加预算 |
|---|---:|---|---|---:|
| `srv-01-cde-api-db` | P0 | PostgreSQL、索引、队列、ModelRouter 和 API 并发导致内存常驻超过 70% | 64GB -> 256GB | ¥40,500 |
| `srv-03-bim-ifc-derivative` | P0 | IFC/STL/STEP/USDZ/OpenUSD 派生、几何缓存、本地推理或 GPU feeding 排队明显 | 64GB -> 256GB | ¥40,500 |
| `srv-02-nas-cde-backup` | P1 | ZFS ARC 命中率低、对象存储/快照/校验影响业务或文件并发上升 | 64GB -> 256GB | ¥40,500 |
| `srv-04-ci-general-worker` | P1 | Office/PDF/CAD 批量解析、CI、测试库和并发 Worker 抢内存 | 64GB -> 128GB/256GB | 128GB +¥13,500；256GB +¥40,500 |
| `srv-05-app-api-queue` | P1 | 应用/API、缓存、Search/Vector 辅助服务和队列并发上升 | 64GB -> 128GB/256GB | 128GB +¥13,500；256GB +¥40,500 |
| `srv-06-jump-log-audit` | P2 | 日志、监控、审计检索和备份校验明显吃内存 | 64GB -> 128GB/256GB | 128GB +¥13,500；256GB +¥40,500 |
| 三台关键节点组合 | - | `srv-01` + `srv-03` + `srv-02` 同时升 256GB | 3 台各加 3 根 64GB | **¥121,500** |
| 全六节点组合 | - | 全部六台升 256GB | 6 台各加 3 根 64GB | **¥243,000** |

### 5.26 AI 生成工程模型、开放格式输出与员工智能体集群

本节把“AI 生成工程模型、输出 IFC/STL/STEP/USDZ/OpenUSD 等开放格式、模型导出施工和加工图纸、模型导出 BOM、BOM 驱动按模计量/材料计划/商务报价、针对业务和员工打造智能体集群”落成系统边界。核心原则不变:AI 可以生成草稿、候选方案、校验建议和待办,不得直接发布可施工、可加工、可采购、可付款、可归档结论。

#### 5.26.0 BIM/CAD/IFC/BOM 专业对象边界

ArchIToken 必须先区分专业对象层级,再谈 AI 生成、数据库落表和业务派生。不能把 Revit 的图元、族、类型、构件实例,IFC 的实体、类型、属性集和关系,钢结构加工件,BOM 行,BOQ 行混成一个“模型构件”。

| 层级 | Revit/CAD/BIM 语义 | IFC/openBIM 语义 | ArchIToken 内部对象 | 典型用途 | 不能混淆为 |
|---|---|---|---|---|---|
| 分类/类别 | Category、图层、对象类别 | IfcClass、classification reference | `classification_ref`, `sjg157_code`, `bsdd_uri` | 检索、字典映射、IDS 条件、BOM 类目 | 不是一个具体构件实例。 |
| 族/类型定义 | Family、Family Type、系统族类型、Tekla profile/part template | `IfcTypeObject`, type property set | `element_type`, `family_ref`, `type_parameters` | 参数模板、规格默认值、材料默认值、可复用构件定义 | 不是现场某一根梁、某一扇门。 |
| 图元/构件实例 | Element Instance、放置后的构件/对象 | `IfcElement`, `IfcProduct`, `IfcGUID` | `model_element` | 工程模型中的可追溯实例,绑定几何、位置、属性、状态和版本 | 不是 BOM 行,也不是族定义。 |
| 属性集 | Instance Parameters、Type Parameters、共享参数 | `IfcPropertySet`, quantity set, material | `property_set`, `quantity_set` | IDS/Validate、BOM、算量、材料、报价 | 不是自由文本备注。 |
| 关系 | host、void、join、assembly、connection | `IfcRel*` 关系 | `model_relation` | 空间归属、构件连接、开洞、聚合、系统关系 | 不是简单父子目录。 |
| 图纸视图 | Sheet、View、detail、annotation | document reference、viewpoint | `drawing_sheet`, `drawing_view`, `detail_ref` | 施工图、加工图、图号、标注、审签 | 不是模型真源。 |
| BOM 行 | 构件清单行、材料清单行 | 由 IFC 对象和属性派生 | `component_bom_line` | 数量、单重、总重、规格、材料、采购/生产引用 | 不是模型对象本身。 |
| BOQ/造价行 | 工程量清单、报价行 | 由 BOM/图纸/规则/价格派生 | `boq_item`, `quote_line` | 算量、损耗、单价、税费、报价版本 | 不是 BOM 行,更不是模型实例。 |

数据库约束: `component_bom_line` 必须可回跳 `model_element_id`、`ifc_guid`、`element_type_id`、`property_set_id`、`drawing_view_id`、`sjg157_code` 和 `bsdd_uri`。Agent 只能生成这些对象的草稿或建议,发布必须经 Validate、专业复核、审批和审计。
...
客服/客户文字/会议/合同/照片/现场条件
  ...
  -> ModelPlanner
  -> element_type / model_element / property_set draft
  -> IFC/STL/STEP/USDZ/OpenUSD format package
...
| 模型规划 | 需求参数、构件库、材料库、连接规则、SJG157 本地编码 + bSDD URI 双挂接 | 拆分楼层、空间、系统、类型定义、构件实例、连接、材料和阶段 | `model_plan`, `component_constraint`, `element_type` | 业务约束冲突项必须生成待确认项或 BCF 模型协同主题。 |
| 模型生成 | `model_plan`, 类型约束, 构件实例约束, 规则库 | 生成类型定义、构件实例、几何引用、属性集、关系和 IFC/STL/STEP/USDZ/OpenUSD 格式包 | `ifc_model`, `model_element`, `element_type`, `property_set`, `model_relation`, `format_package` | 不能直接发布；必须经 IFC/IDS/规则/语义校验。 |
| 施工图派生 | 已校验模型、图纸模板、专业规则 | 生成平面、立面、剖面、节点、安装定位、图号和标注草稿 | `construction_drawing`, `drawing_sheet` | 专业负责人复核、图纸版本、模型引用和审批齐全后发布。 |
| 加工图派生 | 已校验模型、构件实例、类型定义、加工规则 | 生成构件编号、下料尺寸、孔位、连接、焊接/紧固、工艺路线和二维码 | `fabrication_drawing`, `fabrication_part` | 无材料等级、尺寸公差、图纸引用或质检规则时不得下发生产。 |
| BOM 派生 | 构件实例、属性集、类型定义、SJG157、bSDD、材料库、图纸索引 | 计算类目、编码、规格、长度、数量、单重、总重、损耗和备注 | `component_bom_line` | 无构件实例、分类、重量、图纸引用或校验结果时阻断发布。 |
...
| 采购/生产/施工 | 已发布 BOM/图纸/材料需求/工单 | 生成请购、排产、下料、质检、包装、发运、安装、整改和验收证据 | `purchase_request`, `work_order`, `acceptance_record` | 未审批、未关闭 BCF 模型协同主题或质检问题、无归档证据时不得完成闭环。 |

#### 5.26.2 模型导出施工图与加工图

| 图纸类型 | 必须包含 | 来源对象 | 禁止事项 |
|---|---|---|---|
| 施工图 | 平面/立面/剖面、轴网、安装定位、节点详图、材料说明、图号、版本、审签、验收点 | `model_element`, `drawing_view`, `drawing_sheet`, `project_stage`, `approval_task` | 不能由 AI 文本直接出发布图；不能脱离构件实例、模型视图和审批签发。 |
...
施工图和加工图是模型派生物,不是新的事实来源。图纸页可以带人工补充标注,但每个构件、尺寸、材料、数量和状态必须能回跳到 `model_element_id`、`ifc_guid`、`property_set_id`、`drawing_view_id`、`bom_line_id` 和审批记录。

#### 5.26.3 BOM 在按模计量、材料计划和商务报价中的应用

| 应用环节 | BOM 如何使用 | 输出 | 校验 |
|---|---|---|---|
| 按模计量/BOQ | 按构件实例、BOM 行、尺寸、数量、重量、损耗和工程量规则生成工程量清单 | `boq_item`, `cost_item` | 必须记录构件实例、IFC GUID、图纸视图、规则版本、单位、价格来源、税费、损耗和差异。 |
| 材料计划 | 按材料等级、规格、长度、库存、供应商、交期和损耗归并 | `material_requisition`, `purchase_request` | 商务报价口径不得直接下单；采购必须基于已发布 BOM/材料需求。 |
| 商务报价 | 从已校验 BOM/BOQ 和价格库生成报价版本,支持客户确认、议价和变更对比 | `quote_version`, `quote_line` | 缺价格来源、成本边界或审批时只能是报价草稿。 |
| 生产加工 | BOM 行拆成加工件、批次、工艺路线、质检项和包装发运对象 | `work_order`, `fabrication_part`, `qc_record` | 加工件必须绑定构件实例、加工图、材料批次和质检记录。 |
| 施工发料 | 按楼层、空间、构件组、安装顺序和现场进度发料 | `site_issue`, `installation_task` | 发料、安装和验收必须回挂构件实例、安装位置和照片/视频证据。 |
...
| `DesignModelAgent` | 方案设计、深化设计、BIM 工程师 | 需求参数、构件库、材料库、连接规则 | GeometryRouter、IFC/STL/STEP/USDZ/OpenUSD exporter、RuleChecker | 类型定义、构件实例、工程模型草稿、开放格式包、属性集 | 不得直接发布模型、图纸或 BOM。 |
| `DrawingBomAgent` | 深化设计、加工技术、BOM 工程师 | 已校验模型、图纸模板、BOM 字段规则 | drawing_export、fabrication_drawing_export、model_export_bom | 施工图草稿、加工图草稿、BOM 草稿 | 不得绕过构件实例、属性集和图纸视图写 BOM 或下发图纸。 |
| `CostQuoteAgent` | 造价、商务报价、经营负责人 | BOM、图纸、BOQ 规则、价格库、税费规则 | boq_calc、quote_builder、change_compare | BOQ、报价版本、成本差异、变更影响 | 不得无价格来源出正式报价。 |
...
| `CollaborationCoordinatorAgent` | 项目经理、部门负责人、个人中心 | 工作流、待办、审批、项目成员、资源冲突 | workflow_query、task_prioritize、blocking_reason | 协同阻塞、升级建议、待办重排建议 | 不得自动审批或替负责人改责任矩阵。 |
| `SchedulingOrchestratorAgent` | 运维、AI 中心、Worker 管理员 | Agent 队列、CPU/GPU/NAS 资源、批处理窗口 | queue_dispatch、resource_lock、capacity_check | 调度计划、限流策略、重试建议 | 不得让 200 个 Agent 同时抢数据库/GPU/NAS。 |
| `DatabaseStewardAgent` | DBA、系统管理员、技术负责人 | 慢查询、连接池、迁移、备份、容量快照 | db_health_check、index_advice、migration_risk | 数据库巡检、索引/分区/扩容建议 | 不得直接执行破坏性 DDL/DML。 |
| `MetadataGovernanceAgent` | 设置治理、标准库、AI 中心 | 元数据对象、字段、Schema、数据字典、规则 | schema_diff、dictionary_check、lineage_trace | 字段冲突、Schema 变更、数据质量待办 | 不得跳过 SchemaValidator 发布字段。 |
| `PriceEvidenceAgent` | 造价、采购、财务 | SKU、店铺、截图、供应商报价、发票和质保 | price_lock_check、evidence_pack、expiry_check | 价格锁定状态、证据缺口、过期提醒 | 不得把无证据价格写成正式采购价。 |
...
|---|---:|---|---|---|
| P0 架构落地 / 0-3 个月 | 24-32 个 | 打通一条真实工程链,证明 AI 员工可控 | 客户需求、设计建模、图纸/BOM、按模计量与商务报价、材料采购、生产施工、档案财务、治理审计 | 不追求全岗位全场景,不做 200 个空壳 Agent。 |
...
#### 5.26.6 200 个 Agent 的设备分布

200 个 Agent 不是 200 个常驻进程,也不是全部放在 GPU 服务器上。它们首先是 `agent_profiles`、`tool_policies`、`workflow_templates`、`model_routes` 和队列任务的注册目录；真正运行时由 `srv-05` 调度,再按任务类型分配到 CPU Worker、BIM/GPU Worker、数据库、CDE/NAS 和审计节点。

| 设备 | 承载内容 | 对应 Agent 类型 | 是否占用 GPU | 边界 |
|---|---|---|---:|---|
| `srv-01-cde-api-db` | Agent Registry、ModelRouter/ToolRouter 元数据、PostgreSQL、权限、审批、状态机和审计事件索引 | 所有 Agent 的配置、身份、权限、状态和结果索引 | 否 | 保存注册和真源状态,不执行重型推理和模型派生。 |
| `srv-05-app-api-queue` | 工作台 API、任务队列、调度器、优先级、重试、限流和成本计量 | 200 个 Agent 的排队、分派、暂停、重试和审批任务 | 否 | 负责调度,不直接生成工程结论。 |
| `srv-04-ci-general-worker` | Office/PDF/CAD 解析、文本抽取、表格处理、报价/成本/采购/财务/人力结构化任务 | 客服、造价、材料、生产、施工、档案、财务、人力、治理类 Agent 的通用 CPU 任务 | 通常否 | 文本/结构化 Agent 不直接占用 BIM GPU；必要时走 ModelRouter 外部或本地小模型。 |
| `srv-03-bim-ifc-derivative` | BIM/IFC/STL/STEP/USDZ/OpenUSD 派生、模型导图、模型导出 BOM、视觉/渲染、本地大模型或几何加速 | `DesignModelAgent`, `DrawingBomAgent`, `IfcIndexAgent`, `DesignRuleCheckAgent`, `TwinSyncAgent` 等工程模型类 Agent | 是,按 GPU 方案 A/B | 只生成草稿和派生物；发布必须经 Validate、规则、专业复核和审批。 |
| `srv-02-nas-cde-backup` | CDE 源文件、格式包、图纸、BOM、归档包、Agent artifact、模型派生产物、备份和冷归档 | 所有 Agent 的输入文件、输出 artifact 和证据包 | 否 | 文件真源和派生物存储,不做业务判断。 |
| `srv-06-jump-log-audit` | JumpServer、会话录像、日志、监控、备份校验、只读审计报表 | Governance、审计、安全、运行健康类 Agent 的日志查询和巡检任务 | 否 | 只能审计、告警和出建议,不得绕过审批改生产数据。 |

容量口径:

| 阶段 | Agent 目录规模 | 并发口径 | 设备策略 |
|---|---:|---|---|
| P0 | 24-32 个 | 少量关键链路串行/低并发 | `srv-05` 调度,`srv-04` 跑文本/结构化,`srv-03` 跑工程模型派生。 |
| P1 | 80-120 个 | 多模块并行,按项目和优先级限流 | `srv-01`/`srv-03` 优先考虑 256GB；必要时增加 Worker。 |
| P2 | 160-200 个 | 目录规模达到 200,但不等于 200 个同时推理任务 | 通过队列、租户限流、GPU 队列、外部模型路由和批处理窗口控制成本。 |
...
| 对象详情 | 需求、构件实例、IFC GUID、属性集、图纸视图、BOM、BOQ、报价、工单、验收之间的回跳链 | `business_object_id`, `object_version_id`, `source_model_element_id`, `ifc_guid`, `property_set_id` |
| 操作队列 | 正在导入、建模、校验、导图、导 BOM、按模计量、报价和归档的后台任务 | `operation_id`, `worker_id`, `status`, `artifact_uri`, `error_code` |
| 审计时间线 | 谁在何时用哪个 Agent/工具做了什么,是否审批,是否回滚 | `audit_event_id`, `actor_id`, `approval_id`, `before`, `after` |

验收标准:

- 输入一段客服需求后,系统能生成需求参数、模型计划、工程模型草稿、IFC/STL/STEP/USDZ/OpenUSD 格式包、施工/加工图草稿、BOM 草稿、BOQ/报价草稿和待审批任务。
- 任意 BOM 行必须能回跳到构件实例、IFC GUID、类型定义、属性集、施工/加工图、分类字典、算量规则、材料需求、报价版本、校验报告、审批记录和 Agent 调用。
- 任意员工 Agent 输出必须带来源、工具、模型路由、输入/输出 hash、规则结果、审批状态和成本审计。
- 缺少专业规则、官方标准、价格来源、构件实例、IFC GUID、属性集、图纸引用、质检证据或审批时,系统只能输出草稿/待确认/启发式建议,不得标记为施工、加工、采购、报价、付款或归档 ready。
...
|---|---|---|
| SJG157 语义字典编码表 | `/home/insome/下载/建筑工程信息模型语义字典编码表_SJG157-2024.xlsx` | 深圳市工程建设地方标准 `SJG 157-2024` 的本地语义字典和类目编码来源。BOM 的“类目名称”和“编码”必须从这里受控选择；同一对象可同时挂 `sjg157_code` 与 `bsdd_uri`。官方发布页: <https://zjj.sz.gov.cn/ztfw/jzjn/gfgl/content/post_11149973.html> |
...
官方来源: 深圳市住房和建设局 2024-02-15 发布《建筑工程信息模型语义字典标准》通知,批准该标准为深圳市工程建设地方标准,编号 `SJG 157-2024`,自 2024-04-01 起实施。它不是和 bSDD 冲突的另一套体系,而是面向本地工程管理、BOM、模型属性和类目编码的 openBIM/IFC 语义本地化发展继承层。ArchIToken 采用“双挂接”策略:国际语义使用 `bsdd_uri`,本地交付和管控使用 `sjg157_code`。
...
| `model_element_id` | 构件实例 ID,必须可关联 IFC GUID 或外部 CAD/BIM element instance ID |
...
| 是否选用 Xeon 696X | 默认不替换基线；`srv-01` 或 `srv-03` 可按需求从 676X 升 696X,每台 +¥24,650；658X 节点升 696X 每台 +¥32,500 | 数据库/API、BIM/模型派生、几何缓存、本地推理或 Worker 队列有明确 CPU 瓶颈,且主板 BIOS/QVL、420W CPU 散热、机箱风道和电源余量通过验收 |
| 是否立即买 BIM GPU 加速卡 | GPU 方案 A 买 2 张 RTX PRO 6000D 84GB 服务器版,小计 ¥136,000；GPU 方案 B 买 4 张 RTX 5090 32GB 双宽涡轮,小计 ¥144,000 | 用于 BIM/IFC/STEP/STL/USDZ/OpenUSD 派生、模型导图、构件 BOM、局部推理和渲染加速；上架前必须验收机箱、PCIe、供电、风道、驱动、质保和温度监控 |
| 是否另买完整 GPU/HA 生产服务器 | 暂不单独采购；先由 `srv-03` BIM 节点承接 GPU 方案 A 或 B 并做上架验收 | 如果 `srv-03` 的机箱、PCIe、双卡 1200W 或四卡约 2300W GPU 供电、风道、驱动、质保或温度监控不满足,再追加 GPU 合规服务器和机房级预算 |
| 是否升级关键节点内存 | `srv-01`、`srv-03`、`srv-02` 可按实际负载优先升到 256GB；单台 64GB -> 256GB 追加 ¥40,500 | 数据库/队列、模型派生、ZFS ARC、Worker 并发或 API 缓存持续吃内存 |
