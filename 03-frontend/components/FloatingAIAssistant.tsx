...
export function FloatingAIAssistant({
  ...
  if (!expanded) {
    return (
      <button
        ...
        className={`arch-surface fixed bottom-4 z-50 flex items-center gap-3 rounded-full border px-4 py-3 text-left backdrop-blur transition hover:scale-[1.02] md:bottom-5 ${
          dock === 'right' ? 'right-4 md:right-5' : 'left-4 md:left-5'
        }`}
        aria-label="展开 ArchIToken AI 助手"
      >
        <span className="arch-btn-primary relative flex h-11 w-11 items-center justify-center rounded-full">
        ...
        <span>
          <span className="arch-text block text-sm font-black">{profile.name}</span>
          <span className="arch-muted block text-xs">在线 · {profile.unread} 条建议</span>
        </span>
      </button>
    );
  }
  ...
  return (
    <>
    <section className={`arch-surface fixed inset-x-3 bottom-20 z-50 max-h-[72vh] overflow-hidden rounded-[2rem] border md:inset-x-auto md:bottom-5 md:w-[370px] ${
      ...
      <div className="arch-border flex items-center justify-between gap-3 border-b px-4 py-3">
        ...
        >
          <span className="arch-btn-primary flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="arch-btn rounded-xl px-2 py-2 text-[10px] font-black transition"
          >
            {dock === 'right' ? '停靠左' : '停靠右'}
          </button>
          <button
            ...
            className="arch-btn flex h-9 w-9 items-center justify-center rounded-xl transition"
      ...
      <div className="max-h-[calc(72vh-74px)] overflow-y-auto p-4">
        ...
        <div className="arch-card-muted mt-4 rounded-[1.5rem] p-4">
          ...
          <div className="mt-3 space-y-2">
            {suggestions.map((item) => (
              <button
                ...
                className="arch-card w-full rounded-2xl px-3 py-2 text-left text-sm leading-6 transition hover:border-[var(--arch-primary)] hover:bg-[var(--arch-primary-soft)]"
              >
                {item}
              </button>
            ))}
        ...
        <div className="arch-card-muted mt-4 rounded-[1.5rem] p-4">
          ...
          <button
            ...
            className="arch-btn-primary mt-3 inline-flex w-full items-center justify-center gap-2 rounded-2xl px-4 py-3 text-sm font-black transition"
          >
    ...
    {chatOpen ? (
      <aside className={`arch-surface fixed bottom-4 top-16 z-40 w-[360px] max-w-[calc(100vw-2rem)] rounded-[2rem] border ${
...
function ProfileCard({
  ...
  const profile = architokenAssistantProfile;
  return (
    <div className="arch-card-muted overflow-hidden rounded-[1.7rem] border">
      <div className="p-4">
        <div className="flex items-start justify-between gap-3">
          <div>
            <div className="arch-btn-primary inline-flex items-center gap-2 rounded-full px-3 py-1 text-xs font-black">
            ...
            <p className="arch-muted mt-2 text-sm leading-6">
              像工程版 xChat / TikTok 主页: 展示作品、能力标签、上下文建议和即时对话入口。
            </p>
          </div>
        ...
        <div className="mt-4 flex flex-wrap gap-2">
          {profile.capabilityTags.map((tag) => (
            <span key={tag} className="arch-chip rounded-full border px-3 py-1 text-xs font-bold">
              {tag}
            </span>
          ))}
        </div>
        ...
        <div className="mt-4 grid grid-cols-2 gap-2">
          {profile.works.slice(0, 6).map((work) => (
            <div key={work.id} className="arch-card rounded-2xl p-3">
        ...
        <div className="mt-4 grid grid-cols-3 gap-2">
          <button
            type="button"
            onClick={onCollect}
            className="arch-btn inline-flex items-center justify-center gap-1 rounded-2xl px-2 py-2 text-xs font-black"
          >
          ...
          <button
            type="button"
            className="arch-btn-primary inline-flex items-center justify-center gap-1 rounded-2xl px-2 py-2 text-xs font-black"
          >
            关注
          </button>
        </div>
