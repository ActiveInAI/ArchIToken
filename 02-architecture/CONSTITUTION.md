...
用户提供的每一个 GitHub 链接都必须先登记到 `docs/ADAPTER_SOURCE_MAP.md`,再进入技术决策或实现。需要运行时能力时,首选从上游 GitHub 源码编译为可审计 sidecar/worker/CLI/service;apt、snap、系统包或二进制分发不可用,不能作为停止理由。
