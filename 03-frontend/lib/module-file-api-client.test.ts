...
describe("module file api client", () => {
  ...
  it("lists module files through the backend API and maps the response", async () => {
    ...
    expect(fetchMock.mock.calls[0]?.[0]).toBe(
      "http://localhost:18080/v1/modules/marketing_service/files?parentScope=root&kind=file&limit=10",
    );
  });
  ...
  it("normalizes frontend root ids and rejects non-backend parents for writes", async () => {
    ...
    expect(
      toBackendParentId(
        "marketing_service",
        getModuleRootId("marketing_service"),
      ),
    ).toBeNull();
      ...
      async (input: RequestInfo | URL, init?: RequestInit) => {
        ...
        const payload = url.includes("/move")
          ? {
              ...backendFile,
              parentId: null,
              metadata: { ...backendFile.metadata, version: 4 },
            }
          : backendFile;
