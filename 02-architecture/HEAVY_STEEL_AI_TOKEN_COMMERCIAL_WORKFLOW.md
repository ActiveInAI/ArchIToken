...
6. 预付定金是业务流程节点,支付方式必须通过支付适配器隔离,支持微信、支付宝、银联、信用卡、PayPal 以及京东、抖音等渠道的可插拔扩展。
...
| 模块 | 关键输入 | 关键输出 | 数据回写 |
|---|---|---|---|
| 市场客服 | 姓名、手机、地理位置、建筑规模、建筑结构、建筑面积、耐火等级、设防烈度、装修风格、备注、资金预算 | 客户需求包、预付定金意向、电子合同草案 | `marketing_requirements`, `prepayment_intents`, CDE 文件 |
...
1. 市场客服提交客户需求后,数据库能查到需求包和预付定金意向。
2. 预付定金流程能进入支付适配器,并生成可审计支付意向,不能只展示按钮。
