...
def test_ifcdb_agent_invalid_json_response_is_artifacted(monkeypatch, tmp_path) -> None:
    monkeypatch.setenv("IFCDB_AGENT_URL", "http://ifcdb-agent.test")
    monkeypatch.setenv("IFCDB_AGENT_VERSION", IFCDB_AGENT_REQUIRED_VERSION)
    ...
    result = run_ifcdb_agent(
        _job(
            ConversionOperation.IFCDB_QUERY,
            {"query": "select * from IfcWall", "outputDir": str(tmp_path)},
        )
    )

    assert result.status == "completed"
    assert result.output["responseKind"] == "binary"
    assert result.artifacts[0].role == "ifcdb_query_invalid_json_response"
    assert (tmp_path / "ifcdb_query_invalid_json_response.txt").read_bytes() == b"{not-json"


def test_ifcdb_agent_multipart_escapes_disposition_values(tmp_path) -> None:
    source = tmp_path / 'bad"name\n.ifc'
    source.write_text("ISO-10303-21;", encoding="utf-8")

    body, content_type = ifcdb_agent_worker._multipart_body(
        "----test-boundary",
        {'field"name\r\n': "value"},
        source,
    )
    ...
    assert 'name="field%22name%0D%0A"' in text
    assert 'filename="bad%22name%0A.ifc"' in text
