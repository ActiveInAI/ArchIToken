...
const hotelDetailedPackages =
  getHeavySteelHotelPackagesForModule("detailed_design");
const hotelDetailedSheets =
  getHeavySteelHotelSheetsForModule("detailed_design");
...
export const moduleSpecs: ModuleSpec[] = [
  {
    ...
    risks: [
      ...
      risk(
        ...
        "合伙人佣金拖欠影响信任链",
        "high",
        "佣金规则必须进入 finance_hr 结算台账,客户签约回款后 7 个工作日内跟踪。",
      ),
    ],
  ...
  },
  {
    ...
    subdomains: [
      {
        id: "hotel-steel-structure",
        name: "重钢装配式钢结构专项",
        purpose:
          "42份结构体系、全栓接节点、模块单元和钢构件加工图，工厂下单前100%冻结。",
        ownerRole: "重钢深化主师",
        capabilityLevel: "automation",
      },
      {
        id: "hotel-architecture-civil",
        name: "建筑土建深化",
        purpose: "25份钢构版建筑平面、装配式隔墙、楼地面、屋面、门窗和细部。",
        ownerRole: "建筑深化设计师",
        capabilityLevel: "workflow",
      },
      {
        id: "hotel-interior-fitout",
        name: "室内精装钢构适配",
        purpose:
          "33份精装模数、吊顶避梁、预埋件、集成卫浴、声桥切断和公区精装。",
        ownerRole: "精装主案",
        capabilityLevel: "workflow",
      },
      {
        id: "hotel-mep-coordination",
        name: "机电综合深化",
        purpose:
          "30份管线综合、穿梁预留、给排水、暖通、强弱电和BA点位，先碰撞后下单。",
        ownerRole: "机电综合设计师",
        capabilityLevel: "automation",
      },
      {
        id: "hotel-envelope-fire",
        name: "围护与消防专项",
        purpose:
          "ALC、外墙一体板、门窗气密水密、热桥断桥、防火包覆和排烟避梁。",
        ownerRole: "围护/消防专项设计师",
        capabilityLevel: "workflow",
      },
      {
        id: "hotel-site-assembly",
        name: "现场装配工艺移交",
        purpose: "18份吊装、调平、螺栓紧固、拼缝防渗、干湿分区和QC清单。",
        ownerRole: "现场钢构代表",
        capabilityLevel: "workflow",
      },
      {
        id: "floor-plan-automation",
        name: "平面自动生成与适配",
        purpose:
          "按外边界、入口点、房间需求和可复用模板生成、适配、家具布置和保存平面候选。",
        ownerRole: "深化设计师",
        capabilityLevel: "automation",
      },
    ],
    ...
    artifacts: [
      artifact(
        "hotel-drawing-catalog-token",
        "198份深化图纸目录 Token",
        "JSON + XLSX",
        "文控经理",
        ["8大专项", "33个分组", "编号/阶段/岗位/优先级"],
      ),
      artifact(
        "q235b-bolted-ifc-model",
        "Q235B全栓接 IFC4.3 模型",
        "IFC",
        "BIM 工程师",
        ["构件编码", "全栓接节点", "600mm模数"],
      ...
      artifact(
        ...
        "BCF + CSV",
        "机电综合设计师",
        ["SS-04-05", "MEP-02-01", "双签确认"],
      ),
      artifact(
        "floor-plan-variant-pack",
        "平面候选与家具布置包",
        "JSON + SVG",
        "深化设计师",
        ["边界", "房间需求", "家具层"],
    ...
    tasks: [
      task(
        "dd-t1",
        `登记 ${hotelDetailedPackages.length} 大专项和 ${hotelDetailedSheets.length} 份深化图纸字段`,
        "文控经理",
        "doing",
      ),
      task(
        "dd-t2",
        `先冻结 ${hotelP1DrawingCount} 份 P1 图纸、孔位和预埋件坐标`,
        "重钢深化主师",
        "todo",
      ),
      task(
        "dd-t3",
        "校核 SS-04-05 与 MEP 穿梁孔、消防排烟穿梁节点一致性",
        "RuleChecker",
        "todo",
      ),
      task(
        "dd-t4",
        "生成并复核平面候选、模板适配和家具布置结果",
        "深化设计师",
        "doing",
      ),
    ],
    ...
    risks: [
      risk(
        ...
        "工厂下单前P1图纸未冻结",
        "critical",
        "重钢专项图纸必须在钢构件下单前100%完成；螺栓孔位、耳板和预留孔洞锁定后不得变更。",
      ),
      risk(
        "dd-r2",
        "穿梁圆孔和机电综合脱节",
        "critical",
        "MEP 穿梁孔必须与 SS-04-05 一一对应并完成机电+钢构双签。",
      ),
      risk(
        "dd-r3",
        "AI输出被误用为施工或报审结论",
        "high",
        "输出状态保持 professional_review_required，结构、消防、节能和施工安全必须人工签审。",
      ),
    ],
    fileTypes: [
      ".ifc",
    ...
    visualization: {
      ...
      telemetry: [
        "drawing_completion_rate",
        "p1_freeze_rate",
        "floor_plan_variant_count",
        "hole_lock_conflict_count",
  ...
  },
  {
    ...
    artifacts: [
      artifact("boq-token", "BOQ Token", "JSON + XLSX", "造价工程师", [
        "模型清单",
        "价格来源",
      ]),
      artifact("cost-baseline", "成本基线", "XLSX", "商务经理", [
        "预算版本",
        "变更记录",
      ]),
    ],
    ...
    risks: [
      risk(
        ...
        "模型清单与报价口径不一致",
        "high",
        "BOQ 项必须保留 element_id 和 rule_id。",
      ),
    ],
  ...
  },
  {
    ...
    status: "planned",
    summary:
      "覆盖造房网合伙人佣金、样板房预算、客户签约回款、项目合同、收付款、发票、成本、班组绩效和组织能力。",
    objective:
      "把 5% 合伙人佣金、回款后7个工作日结算、样板房55万-74万元投入、项目生产成本和签约后服务转成可核算、可付款、可审计的 Enterprise Token。",
    subdomains: [
      {
        id: "contract-ledger",
        name: "合同台账",
        purpose: "管理主合同、分包合同、补充协议、履约节点和合同风险。",
        ownerRole: "商务经理",
        capabilityLevel: "workflow",
      },
      {
        ...
        name: "合伙人佣金",
        purpose: "管理5%佣金、回款后7个工作日结算和推荐奖励。",
        ownerRole: "渠道财务",
        capabilityLevel: "workflow",
      },
      {
        id: "sample-house-budget",
        name: "样板房预算",
        purpose:
          "跟踪146㎡占地、224㎡建筑面积、55万-74万元展示样板房预算拆解。",
        ownerRole: "财务经理",
        capabilityLevel: "workflow",
      },
      {
      ...
      },
      {
        id: "cost-control",
        name: "成本控制",
        purpose: "对接 BOQ、采购、生产、施工和变更形成动态成本。",
        ownerRole: "成本经理",
        capabilityLevel: "automation",
      },
      {
        id: "hr-crew",
        name: "人员与班组",
        purpose: "管理项目人员、班组、资质、考勤、工效和安全培训。",
        ownerRole: "人力资源经理",
        capabilityLevel: "workflow",
      },
      {
        id: "performance",
        name: "绩效与结算",
        purpose: "按进度、质量、安全、成本和交付物完成度形成绩效与结算依据。",
        ownerRole: "项目负责人",
        capabilityLevel: "simulation",
      },
    ],
    ...
    outputs: ["digital_archive", "settings_center"],
    artifacts: [
      artifact("enterprise-token", "Enterprise Token", "JSON", "财务经理", [
        "合同台账",
        "付款记录",
      ]),
      artifact(
      ...
      artifact(
        "sample-house-budget-pack",
        "样板房预算拆解包",
        "XLSX + PDF",
        "财务经理",
        ["主体30万", "基础5万", "设备5万", "精装15万"],
      ),
      artifact("cashflow-report", "现金流报告", "XLSX + PDF", "商务经理", [
        "成本基线",
        "收付款计划",
      ]),
      artifact(
        "crew-performance-pack",
        "班组绩效包",
        "PDF + JSON",
        "项目经理",
        ["考勤", "质量安全记录"],
  ...
  },
  {
    ...
    status: "foundation",
    summary:
      "租户、RBAC、模型路由、SLA、规范库版本、存储策略、审计和 UI 偏好的平台控制中心。",
    objective:
      "让模块、Agent、Router、Schema、规则和权限都可配置、可审计、可回滚。",
    subdomains: [
      {
        id: "tenant-rbac",
        name: "租户与 RBAC",
        purpose: "管理组织、角色、权限和隔离策略。",
        ownerRole: "平台管理员",
        capabilityLevel: "foundation",
      },
      {
        id: "model-routing",
        name: "模型路由",
        purpose: "配置 ModelRouter、InferenceRouter 和成本策略。",
        ownerRole: "AI 平台工程师",
        capabilityLevel: "workflow",
      },
      {
        id: "sla-policy",
        name: "SLA 与回滚",
        purpose: "管理模块 SLA、超时、重试和 RollbackGuard。",
        ownerRole: "SRE",
        capabilityLevel: "workflow",
      },
      {
        id: "schema-rules",
        name: "Schema / 规则版本",
        purpose: "管理 Module Schema、JSON Schema 和规则版本。",
        ownerRole: "架构师",
        capabilityLevel: "foundation",
      },
    ],
    ...
    artifacts: [
      artifact("governance-token", "Governance Token", "JSON", "平台管理员", [
        "策略版本",
        "审计日志",
      ]),
      artifact("route-table", "模型与工具路由表", "YAML", "AI 平台工程师", [
        "成本预算",
        "fallback",
      ]),
    ],
