...
export interface ModuleBackendAdapter {
  ...
  openFile(fileId: string): { node: ModuleFileNode; auditEvent: ModuleAuditEvent };
  openUploadedFile(fileId: string): { metadata: LocalFileMetadata; auditEvent: ModuleAuditEvent };
  createFile(input: CreateFileInput): { node: ModuleFileNode; auditEvent: ModuleAuditEvent };
  uploadFile(input: UploadFileInput): { node: ModuleFileNode; auditEvent: ModuleAuditEvent };
  uploadLocalFile(metadata: LocalFileMetadata, parentId: string): { node: ModuleFileNode; transaction: ModuleTransaction; auditEvent: ModuleAuditEvent };
  downloadFile(fileId: string): { job: ModuleDownloadJob; auditEvent: ModuleAuditEvent };
  moveFile(fileId: string, targetParentId: string): { node: ModuleFileNode; auditEvent: ModuleAuditEvent };
  copyFile(fileId: string): { clipboard: ModuleClipboard; auditEvent: ModuleAuditEvent };
  pasteFile(moduleId: ModuleId, targetParentId: string): { nodes: ModuleFileNode[]; auditEvent: ModuleAuditEvent };
  shareFile(fileId: string): { link: ModuleShareLink; auditEvent: ModuleAuditEvent };
  deleteFile(fileId: string): { node: ModuleFileNode; auditEvent: ModuleAuditEvent };
  renameFile(fileId: string, name: string): { node: ModuleFileNode; auditEvent: ModuleAuditEvent };
  getProperties(fileId: string): { node: ModuleFileNode; auditEvent: ModuleAuditEvent };
  createTransaction(input: CreateTransactionInput): { transaction: ModuleTransaction; auditEvent: ModuleAuditEvent };
  transitionTransaction(transactionId: string, event: ModuleTransactionEvent): { transaction: ModuleTransaction; auditEvent: ModuleAuditEvent };
  approveTransaction(transactionId: string, approver: string, comment: string): { transaction: ModuleTransaction; auditEvent: ModuleAuditEvent };
  rejectTransaction(transactionId: string, approver: string, comment: string): { transaction: ModuleTransaction; auditEvent: ModuleAuditEvent };
...
export class SessionModuleBackendAdapter implements ModuleBackendAdapter {
  ...
  snapshot(moduleId?: ModuleId): ModuleBackendSnapshot {
    return {
      files: moduleId ? this.files.filter((file) => file.moduleId === moduleId) : [...this.files],
      uploadedFiles: moduleId
        ? this.uploadedFiles.filter((file) => file.moduleId === moduleId)
        : [...this.uploadedFiles],
      transactions: moduleId
        ? this.transactions.filter((transaction) => transaction.moduleId === moduleId)
        : [...this.transactions],
      auditEvents: moduleId ? this.listAuditEvents(moduleId) : [...this.auditEvents],
  ...
  listFiles(moduleId: ModuleId, parentId: string): ModuleFileNode[] {
    return this.files
      .filter((file) => file.moduleId === moduleId && file.parentId === parentId)
      .sort((left, right) => {
  ...
  openFile(fileId: string): { node: ModuleFileNode; auditEvent: ModuleAuditEvent } {
    ...
    const auditEvent = this.record(node.moduleId, 'FileExplorer', `打开 ${node.type === 'folder' ? '文件夹' : '文件'} ${node.name}`);
  ...
  openUploadedFile(fileId: string): { metadata: LocalFileMetadata; auditEvent: ModuleAuditEvent } {
    ...
    const auditEvent = this.record(metadata.moduleId, 'LocalFileRuntimeAdapter', `查看本地上传文件 ${metadata.originalName}`);
    return { metadata, auditEvent };
  }

  createFile(input: CreateFileInput): { node: ModuleFileNode; auditEvent: ModuleAuditEvent } {
    const auditEvent = this.record(input.moduleId, 'FileExplorer', `新建 ${input.type === 'folder' ? '文件夹' : '文件'} ${input.name}`);
    const node = this.buildNode(input.moduleId, input.parentId, input.name, input.type, 'active', auditEvent);
  ...
  uploadLocalFile(metadata: LocalFileMetadata, parentId: string): { node: ModuleFileNode; transaction: ModuleTransaction; auditEvent: ModuleAuditEvent } {
    const auditEvent = this.record(metadata.moduleId, 'LocalFileRuntimeAdapter', `本地上传 ${metadata.originalName}`);
    ...
    this.files = [node, ...this.files.filter((file) => file.localFileId !== metadata.fileId)];
    this.uploadedFiles = [metadata, ...this.uploadedFiles.filter((file) => file.fileId !== metadata.fileId)];
    const transaction = this.createUploadTransaction(metadata.moduleId, metadata.originalName, node.id, auditEvent);
  ...
  downloadFile(fileId: string): { job: ModuleDownloadJob; auditEvent: ModuleAuditEvent } {
    const node = this.requireFile(fileId);
    const auditEvent = this.record(node.moduleId, 'FileExplorer', `下载任务已创建 ${node.name}`);
  ...
  moveFile(fileId: string, targetParentId: string): { node: ModuleFileNode; auditEvent: ModuleAuditEvent } {
    ...
    const auditEvent = this.record(node.moduleId, 'FileExplorer', `移动 ${node.name} 到 ${target.name}`);
    this.updateFile(fileId, { parentId: targetParentId, status: 'moved' }, auditEvent);
  ...
  copyFile(fileId: string): { clipboard: ModuleClipboard; auditEvent: ModuleAuditEvent } {
    const node = this.requireFile(fileId);
    const auditEvent = this.record(node.moduleId, 'FileExplorer', `复制 ${node.name} 到剪贴板`);
    this.clipboard = { sourceFileId: fileId, sourceName: node.name, mode: 'copy' };
  ...
  pasteFile(moduleId: ModuleId, targetParentId: string): { nodes: ModuleFileNode[]; auditEvent: ModuleAuditEvent } {
    if (!this.clipboard) {
      const auditEvent = this.record(moduleId, 'FileExplorer', '粘贴失败: 剪贴板为空');
      return { nodes: [], auditEvent };
    }
    const source = this.requireFile(this.clipboard.sourceFileId);
    const target = this.requireFile(targetParentId);
    const auditEvent = this.record(moduleId, 'FileExplorer', `粘贴 ${source.name} 到 ${target.name}`);
  ...
  shareFile(fileId: string): { link: ModuleShareLink; auditEvent: ModuleAuditEvent } {
    const node = this.requireFile(fileId);
    const auditEvent = this.record(node.moduleId, 'FileExplorer', `分享链接已生成 ${node.name}`);
  ...
  deleteFile(fileId: string): { node: ModuleFileNode; auditEvent: ModuleAuditEvent } {
    const node = this.requireFile(fileId);
    const auditEvent = this.record(node.moduleId, 'FileExplorer', `软删除 ${node.name}`);
  ...
  renameFile(fileId: string, name: string): { node: ModuleFileNode; auditEvent: ModuleAuditEvent } {
    const node = this.requireFile(fileId);
    const auditEvent = this.record(node.moduleId, 'FileExplorer', `重命名 ${node.name} 为 ${name}`);
    this.updateFile(fileId, { name, mimeType: mimeForName(name, node.type) }, auditEvent);
    this.touchTransaction(node.moduleId, auditEvent, fileId);
    return { node: this.requireFile(fileId), auditEvent };
  }
  ...
  getProperties(fileId: string): { node: ModuleFileNode; auditEvent: ModuleAuditEvent } {
    const node = this.requireFile(fileId);
    const auditEvent = this.record(node.moduleId, 'FileExplorer', `查看属性 ${node.name}`);
    this.touchFile(fileId, auditEvent);
    return { node: this.requireFile(fileId), auditEvent };
  }

  createTransaction(input: CreateTransactionInput): { transaction: ModuleTransaction; auditEvent: ModuleAuditEvent } {
    const auditEvent = this.record(input.moduleId, 'Lifecycle', `创建事务 ${input.type}`);
  ...
  transitionTransaction(transactionId: string, event: ModuleTransactionEvent): { transaction: ModuleTransaction; auditEvent: ModuleAuditEvent } {
    ...
    const auditEvent = updated.auditTrail[0] ?? createLifecycleAudit('Lifecycle', `${event}`);
  ...
  approveTransaction(transactionId: string, approver: string, comment: string): { transaction: ModuleTransaction; auditEvent: ModuleAuditEvent } {
    ...
    const auditEvent = updated.auditTrail[0] ?? createLifecycleAudit(approver, 'approve');
    this.replaceTransaction(updated);
    this.auditEvents = [auditEvent, ...this.auditEvents].slice(0, 80);
    return { transaction: updated, auditEvent };
  }
  ...
  rejectTransaction(transactionId: string, approver: string, comment: string): { transaction: ModuleTransaction; auditEvent: ModuleAuditEvent } {
    ...
    const auditEvent = updated.auditTrail[0] ?? createLifecycleAudit(approver, 'reject');
    this.replaceTransaction(updated);
    this.auditEvents = [auditEvent, ...this.auditEvents].slice(0, 80);
    return { transaction: updated, auditEvent };
  }
  ...
  private buildNode(
    ...
    const node: ModuleFileNode = {
      id: localFile ? `uploaded-${localFile.fileId}` : `${moduleId}-${type}-${Date.now()}-${Math.random().toString(16).slice(2)}`,
      name,
      type,
      moduleId,
      parentId,
  ...
  private createUploadTransaction(
    ...
    (['submit', 'generate', 'evaluate', 'rule_check', 'validate_schema', 'request_approval'] as const).forEach((event) => {
      transaction = transitionModuleTransaction(transaction, event, 'LocalFileRuntimeAdapter');
    });
    return transaction;
  }

  private cloneNodeTree(source: ModuleFileNode, targetParentId: string, auditEvent: ModuleAuditEvent): ModuleFileNode[] {
    ...
    const descendants = this.files.filter((file) => file.parentId === source.id);
    return [
      clone,
      ...descendants.flatMap((child) => this.cloneNodeTree(child, cloneId, auditEvent)),
    ];
  }

  private record(moduleId: ModuleId, actor: string, summary: string): ModuleAuditEvent {
  ...
  private updateFile(fileId: string, patch: Partial<ModuleFileNode>, auditEvent: ModuleAuditEvent) {
    this.files = this.files.map((file) =>
      file.id === fileId
        ? {
            ...file,
  ...
  private touchTransaction(moduleId: ModuleId, auditEvent: ModuleAuditEvent, relatedFileId?: string) {
    const transaction = this.transactions.find((item) => item.moduleId === moduleId);
    if (!transaction) {
      return;
    }
    const relatedFileIds = relatedFileId && !transaction.relatedFileIds.includes(relatedFileId)
