...
| DWG / RVT / SKP | licensed service such as Autodesk APS, approved DWG adapter, or legal SketchUp reader/exporter. SKP uses the `docs/SKETCHUP_SKP_SIDECAR.md` contract: SketchUp Ruby `Model#export`, isolated GPL BIM-Tools IFC Manager for IFC, Yulio glTF exporter for GLB, or Speckle SketchUp sidecar. SKP->IFC may use `PANAEC_SKP_TO_IFC_COMMAND`; SKP->GLB preview may use `PANAEC_SKP_CONVERTER_COMMAND`. | `blocked` until service URL or command is configured; SKP->IFC never falls back to GLB |
