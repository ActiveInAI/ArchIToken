...
每个员工在统一工作台拥有岗位 Copilot；流程 Agent 串联客服、设计、BIM、造价、材料、生产、施工、档案和财务，但所有工程结论仍由规则、专业复核和人工审批发布。

> 编排链：客服需求 Agent -> 设计建模 Agent -> 图纸/BOM Agent -> 算量报价 Agent -> 材料采购 Agent -> 生产施工档案 Agent -> Approver；岗位 Agent 只生成草稿、校验建议和待办，Generator 和 Evaluator 必须分离。
...
# 10. 6 台 CPU 服务器 + 2 张 RTX PRO 6000D

一期 CPU 核心物料先支撑 30 人内部生产力和 100/1000 用户试点；BIM 节点用长城黑匣子 3200W 电源和长城黑匣子 15 机箱，其它非 NAS 节点按服务器机箱/电源各 ¥1,000。

> 部署设计：srv-01 跑 CDE/API/数据库；srv-02 跑 NAS/备份/CDE 文件；srv-03 跑 BIM/IFC/STEP/STL/USDZ/OpenUSD 模型派生并承接 2 张 RTX PRO 6000D；srv-04 跑 CI/通用 Worker；srv-05 跑应用/API/任务队列；srv-06 跑 JumpServer/日志/审计/监控。CPU 配置为 2 x Xeon 676X + 4 x Xeon 658X，每台至少 64GB ECC RDIMM。
...
**汇报重点**: 硬件结论：一期用 6 台 CPU 服务器拆分 CDE、数据库、NAS、Worker、应用队列、堡垒机、日志和备份；BIM GPU 另购 2 张 RTX PRO 6000D，但必须通过服务器机箱、PCIe、供电、风道、驱动和温度监控验收。