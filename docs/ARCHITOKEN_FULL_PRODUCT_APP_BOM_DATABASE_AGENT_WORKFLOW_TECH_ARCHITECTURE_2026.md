...
| 硬件预算 | 40-50 万 CPU/存储/网络包不能同时覆盖完整 HA 生产机房。 | CPU 基础包与 BIM GPU 专项分账:当前按 2 台 676X + 4 台 658X 六节点轻量试点包；另购 2 张 RTX PRO 6000D 84GB 服务器版显卡,显卡小计 ¥136,000；公网生产和 HA 仍单独立项。 | P0 |
...
| Agent 编排与岗位任务队列 | `srv-05` 队列 + `srv-04` 通用 Worker + `srv-03` 模型派生 Worker | 一期先承载 24-32 个关键 Agent；P1 扩展 80-120 个 Agent 任务目录；文本/结构化 Agent 不直接占用 BIM GPU。 |
...
| 是否另买完整 GPU/HA 生产服务器 | 暂不单独采购；先由 `srv-03` BIM 节点承接 2 张 GPU 并做上架验收 | 如果 `srv-03` 的机箱、PCIe、双卡 1200W 供电、风道、驱动或温度监控不满足,再追加 GPU 合规服务器和机房级预算 |
