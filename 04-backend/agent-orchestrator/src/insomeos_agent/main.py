...
from .phases import get_runner
from .settings import get_settings
from .state import AgentRequest, AgentResponse, BusinessPhase, PhaseState, Verdict
...
app = FastAPI(
    title="InsomeOS Agent Orchestrator",
    version=__version__,
    description="L4 · LangGraph 3-role Harness for the 9 AEC phases",
    lifespan=lifespan,
)
...
async def invoke(req: AgentRequest) -> AgentResponse:
    """Run one business phase end-to-end and return the verdict."""
    request_id = uuid.uuid4().hex
    try:
        runner = get_runner(req.phase)
    except KeyError as exc:
        raise HTTPException(
            status_code=400, detail=f"unknown phase: {req.phase}"
        ) from exc
...
@app.get("/v1/phases")
async def list_phases() -> dict[str, list[str]]:
    return {"phases": [p.value for p in BusinessPhase]}


def main() -> None:
    cfg = get_settings()
    uvicorn.run(
        "insomeos_agent.main:app",
        host=cfg.host,
        port=cfg.port,
