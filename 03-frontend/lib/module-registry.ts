...
import {
  ...
  getHeavySteelHotelPackagesForModule,
...
const hotelDrawingTotal = heavySteelHotelProgram.totalDrawings;
const hotelP1DrawingCount = heavySteelHotelProgram.phaseCounts['P1·先行'];
const hotelHighPriorityDrawingCount = heavySteelHotelProgram.priorityCounts['高'];
const hotelDetailedPackages = getHeavySteelHotelPackagesForModule('detailed_design');
const hotelDetailedSheets = getHeavySteelHotelSheetsForModule('detailed_design');
const hotelProductionSheets = getHeavySteelHotelSheetsForModule('production_manufacturing');
const hotelConstructionSheets = getHeavySteelHotelSheetsForModule('construction_management');
const zaofangMarketingStageCount = zaofangMarketingStages.length;
const zaofangMarketingStagesForMarketing = getZaofangMarketingStagesForModule('marketing_service');
...
export const moduleSpecs: ModuleSpec[] = [
  {
    ...
    objective: '把官网/落地页、企微承接、合伙人档案、样板房体验、标杆案例和签约后服务沉淀为可追溯 Lead Token,不得用虚假承诺替代证据。',
    subdomains: [
      { id: 'landing-page', name: '官网/落地页', purpose: '承载8项核心内容、企微二维码和预约看房入口。', ownerRole: '增长负责人', capabilityLevel: 'workflow' },
      { id: 'wechat-intake', name: '企业微信承接', purpose: '自动欢迎语、避坑手册、预约系统和客户标签体系。', ownerRole: '企微顾问', capabilityLevel: 'automation' },
      { id: 'zero-partner', name: '0号合伙人', purpose: '筛选每城至少1个高信任本地种子合伙人并签约。', ownerRole: '合伙人经理', capabilityLevel: 'workflow' },
      { id: 'partner-network', name: '合伙人网络', purpose: '拓展设计机构、包工头、商会负责人和乡村意见领袖。', ownerRole: '渠道负责人', capabilityLevel: 'workflow' },
      { id: 'sample-house', name: '样板房体验', purpose: '用实景、AR透视、型材样品、数据看板和工艺展示完成转化。', ownerRole: '样板房接待', capabilityLevel: 'simulation' },
      { id: 'case-spread', name: '标杆案例传播', purpose: '形成本地案例视频、图片和合伙人圈层转发证据。', ownerRole: '内容运营', capabilityLevel: 'automation' },
      { id: 'after-sign-service', name: '签约后服务', purpose: '每周同步施工进度并引导客户二次裂变。', ownerRole: '客户成功', capabilityLevel: 'workflow' },
    ],
    inputs: [],
    outputs: ['planning_management', 'concept_design', 'finance_hr', 'digital_archive'],
    artifacts: [
      artifact('zaofang-lead-token', '造房网 Lead Token', 'JSON + PDF', '合伙人经理', ['企微加微记录', '客户标签', '预约看房']),
      artifact('partner-network-register', '10-15个合伙人网络档案', 'XLSX + JSON', '渠道负责人', ['合伙人协议', '资源类型', '签约时间']),
      artifact('sample-house-conversion-pack', '样板房10步转化包', 'PDF + Video + JSON', '样板房接待', ['AR透视记录', '预算测算表', '签约记录']),
      artifact('benchmark-case-media-pack', '本地标杆案例传播包', 'MP4 + JPG + Copy', '内容运营', ['客户采访', '施工过程素材', '转发记录']),
    ],
    workflowStates: workflow('marketing_service'),
    agentGates: gates(),
    tasks: [
      task('ms-t1', '完成三城重点乡镇和样板房选址核验', '增长负责人', 'doing'),
      task('ms-t2', `锁定 ${zaofangMarketingStagesForMarketing.length} 段推广执行证据字段`, 'Planner Agent', 'todo'),
      task('ms-t3', '建立合伙人服务群、档案和佣金结算追踪', '渠道负责人', 'todo'),
    ],
    approvals: [
    ...
    risks: [
      risk('ms-r1', '夸大宣传或虚假承诺', 'critical', '页面、口播、合伙人话术和接待话术必须保留证据,不得承诺超出政策范围的补贴、工期或收益。'),
      risk('ms-r2', '合伙人佣金拖欠影响信任链', 'high', '佣金规则必须进入 finance_hr 结算台账,客户签约回款后 7 个工作日内跟踪。'),
    ],
    fileTypes: ['.pdf', '.docx', '.jpg', '.mp4', '.xlsx', '.json', '.qr'],
    visualization: {
      mode: 'timeline',
      title: '造房网 60 天合伙人推广漏斗',
      layers: ['前期准备', '线上冷启动', '0号合伙人', '合伙人赋能', '标杆案例', '样板房成交', '裂变复盘'],
      telemetry: ['partner_signed_count', 'wechat_leads', 'sample_house_visits', 'contract_signed_count', 'commission_settlement_sla'],
    },
    standards: ['PIPL consent record', '广告宣传真实性审查', '合同授权与电子签章', 'PMBOK stakeholder register', '佣金结算内控'],
    dataObjects: ['leads', 'contacts', 'wechat_intakes', 'partner_profiles', 'partner_agreements', 'sample_house_visits', 'case_media_assets', 'commission_records'],
  ...
  },
  {
    ...
    track: 'governance',
    status: 'pilot',
    summary: '把市场线索、合同边界、设计任务和施工目标转成项目立项、WBS、里程碑、资源计划、风险、RACI 和项目管理图表。',
    objective: '形成可被方案设计、造价、生产、施工和财务共同消费的 Project Plan Token,并支持在线编制、保存版本、审批归档和导出。',
    subdomains: [
    ...
    artifacts: [
      artifact('project-plan-token', 'Project Plan Token', 'JSON + CDE', '项目经理', ['立项资料', '责任矩阵', '图表模板']),
      artifact('wbs-baseline', 'WBS/CBS 基线', 'XLSX + JSON', '计划工程师', ['里程碑', '资源约束']),
      artifact('planning-diagrams', '计划图表包', 'ARCHIPLAN + SVG + CSV + MMD', '计划工程师', ['甘特', 'WBS', 'RACI', '风险矩阵']),
    ...
    fileTypes: ['.archiplan.json', '.mpp', '.xer', '.xlsx', '.csv', '.pdf', '.json', '.yaml', '.mmd', '.bpmn', '.svg'],
    visualization: {
      mode: 'timeline',
      title: 'Project Planning Studio · 项目计划、WBS、图表和里程碑总控',
      layers: ['立项', 'WBS', 'CBS', '甘特', 'PERT', 'RACI', '看板', '资源', '审批', '风险'],
      telemetry: ['milestone_readiness', 'resource_conflict_count', 'approval_delay', 'critical_path_length', 'diagram_version_count'],
    },
  ...
  },
  {
    ...
    track: 'design',
    status: 'pilot',
    summary: `${heavySteelHotelProgram.projectTitle} 深化工作台: ${heavySteelHotelDrawingPackages.length} 大专项、${heavySteelHotelProgram.sectionCount} 个分组、${hotelDrawingTotal} 份图纸明细，覆盖 P1/P2/P3 全周期。`,
    objective: `按 ${heavySteelHotelProgram.structureSystem}、${heavySteelHotelProgram.precisionRule} 和 ${heavySteelHotelProgram.modularRule} 输出可被计量、制造、施工和孪生消费的 Design Token。`,
    subdomains: [
      { id: 'hotel-steel-structure', name: '重钢装配式钢结构专项', purpose: '42份结构体系、全栓接节点、模块单元和钢构件加工图，工厂下单前100%冻结。', ownerRole: '重钢深化主师', capabilityLevel: 'automation' },
      { id: 'hotel-architecture-civil', name: '建筑土建深化', purpose: '25份钢构版建筑平面、装配式隔墙、楼地面、屋面、门窗和细部。', ownerRole: '建筑深化设计师', capabilityLevel: 'workflow' },
      { id: 'hotel-interior-fitout', name: '室内精装钢构适配', purpose: '33份精装模数、吊顶避梁、预埋件、集成卫浴、声桥切断和公区精装。', ownerRole: '精装主案', capabilityLevel: 'workflow' },
      { id: 'hotel-mep-coordination', name: '机电综合深化', purpose: '30份管线综合、穿梁预留、给排水、暖通、强弱电和BA点位，先碰撞后下单。', ownerRole: '机电综合设计师', capabilityLevel: 'automation' },
      { id: 'hotel-envelope-fire', name: '围护与消防专项', purpose: 'ALC、外墙一体板、门窗气密水密、热桥断桥、防火包覆和排烟避梁。', ownerRole: '围护/消防专项设计师', capabilityLevel: 'workflow' },
      { id: 'hotel-site-assembly', name: '现场装配工艺移交', purpose: '18份吊装、调平、螺栓紧固、拼缝防渗、干湿分区和QC清单。', ownerRole: '现场钢构代表', capabilityLevel: 'workflow' },
    ],
    ...
    artifacts: [
      artifact('hotel-drawing-catalog-token', '198份深化图纸目录 Token', 'JSON + XLSX', '文控经理', ['8大专项', '33个分组', '编号/阶段/岗位/优先级']),
      artifact('q235b-bolted-ifc-model', 'Q235B全栓接 IFC4.3 模型', 'IFC', 'BIM 工程师', ['构件编码', '全栓接节点', '600mm模数']),
      artifact('p1-freeze-pack', 'P1先行冻结包', 'PDF + DWG + BCF', '重钢深化主师', ['55份P1图纸', '孔位坐标', '穿梁圆孔锁定']),
      artifact('mep-hole-bcf-pack', 'MEP穿梁孔位与碰撞闭环包', 'BCF + CSV', '机电综合设计师', ['SS-04-05', 'MEP-02-01', '双签确认']),
    ],
    workflowStates: workflow('detailed_design'),
    agentGates: gates(),
    tasks: [
      task('dd-t1', `登记 ${hotelDetailedPackages.length} 大专项和 ${hotelDetailedSheets.length} 份深化图纸字段`, '文控经理', 'doing'),
      task('dd-t2', `先冻结 ${hotelP1DrawingCount} 份 P1 图纸、孔位和预埋件坐标`, '重钢深化主师', 'todo'),
      task('dd-t3', '校核 SS-04-05 与 MEP 穿梁孔、消防排烟穿梁节点一致性', 'RuleChecker', 'todo'),
    ],
    approvals: [approval('dd-a1', '深化模型出图审批', '设计总工')],
    risks: [
      risk('dd-r1', '工厂下单前P1图纸未冻结', 'critical', '重钢专项图纸必须在钢构件下单前100%完成；螺栓孔位、耳板和预留孔洞锁定后不得变更。'),
      risk('dd-r2', '穿梁圆孔和机电综合脱节', 'critical', 'MEP 穿梁孔必须与 SS-04-05 一一对应并完成机电+钢构双签。'),
      risk('dd-r3', 'AI输出被误用为施工或报审结论', 'high', '输出状态保持 professional_review_required，结构、消防、节能和施工安全必须人工签审。'),
    ],
    fileTypes: ['.ifc', '.ids', '.bcf', '.dwg', '.dxf', '.pdf', '.step', '.glb', '.xlsx', '.csv'],
    visualization: {
      mode: 'model',
      title: '100间精品酒店 · 198份深化图纸与IFC/MBD联动视口',
      layers: ['8大专项', 'P1冻结', '全栓接节点', '穿梁圆孔', '消防围护', '现场装配工艺', 'BCF状态'],
      telemetry: ['drawing_completion_rate', 'p1_freeze_rate', 'hole_lock_conflict_count', 'high_priority_open_count'],
    },
    standards: ['IFC4.3', 'IDS', 'BCF', 'GB 50017', 'GB 50011', 'GB 50009', 'GB 50205', 'GB 50661', 'ISO 19650'],
    dataObjects: ['bim_models', 'drawing_catalog_items', 'steel_connection_details', 'bolt_hole_coordinates', 'mep_penetrations', 'structure_calculations', 'clash_reports', 'signoff_records'],
  ...
  },
  {
    ...
    objective: '把 SS-04 钢构件加工图、SS-04-04 孔位坐标、SS-04-08 BOM、SS-04-11 构件运输编码和防火防腐工艺转成可制造、可追溯、可质检、可发运的生产系统。',
    subdomains: [
      { id: 'p1-manufacturing-freeze', name: 'P1生产下单闸门', purpose: '钢构专项、螺栓孔位、耳板尺寸和预留孔洞未冻结不得下单。', ownerRole: '生产负责人', capabilityLevel: 'workflow' },
      { id: 'component-shop-drawings', name: '构件加工详图', purpose: '钢柱、主梁、次梁每型下料长度、孔位坐标、公差0~-2mm和端头处理。', ownerRole: '制图专员', capabilityLevel: 'automation' },
      { id: 'bolt-hole-cnc', name: 'CNC/数控文件', purpose: '统一坐标系输出圆孔孔位、孔径公差+0.5mm和数控钻孔数据。', ownerRole: '数控工程师', capabilityLevel: 'automation' },
      { id: 'factory-welding-only', name: '仅工厂焊接', purpose: '端板、加劲肋、吊耳和节点板在工厂完成，现场只允许栓接。', ownerRole: '焊接工程师', capabilityLevel: 'workflow' },
      { id: 'coating-fireproofing', name: '防腐防火涂装', purpose: '底漆环氧富锌70μm、中漆环氧云铁100μm、面漆聚氨酯60μm和耐火极限分区。', ownerRole: '涂装工程师', capabilityLevel: 'workflow' },
      { id: 'bom-package-code', name: 'BOM与包装编码', purpose: '构件编号、规格、单重、Q235B材质、出厂顺序、包装单元和二维码追溯。', ownerRole: '物料统筹人员', capabilityLevel: 'workflow' },
      { id: 'factory-qc', name: '工厂质检放行', purpose: '尺寸、孔位、栓接件、涂层、防火和出厂资料齐套后放行。', ownerRole: '质检工程师', capabilityLevel: 'automation' },
      { id: 'mes-erp', name: 'MES/ERP 对接', purpose: '同步工单、排产、库存和成本。', ownerRole: '系统集成工程师', capabilityLevel: 'workflow' },
      { id: 'coding-shipping', name: '构件编码 / 包装发运 / 返工', purpose: '按构件号-方向-楼层-区域喷涂编号并绑定包装、装车、到场签收和返工处理。', ownerRole: '车间主任', capabilityLevel: 'workflow' },
    ],
    inputs: ['detailed_design', 'quantity_costing', 'standard_library'],
    outputs: ['material_logistics', 'construction_management', 'digital_twin'],
    artifacts: [
      artifact('hotel-production-release-token', '酒店重钢生产放行 Token', 'JSON', '生产负责人', ['P1冻结记录', '模型版本', 'BOM']),
      artifact('bolt-hole-cnc-package', '螺栓孔位CNC包', 'NC + DXF + CSV', '数控工程师', ['SS-04-04', '圆孔孔径', '孔径公差+0.5mm']),
      artifact('q235b-bom-package', 'Q235B构件BOM与包装单', 'XLSX + QR', '物料统筹人员', ['SS-04-08', 'SS-04-11', '出厂顺序']),
      artifact('coating-fireproof-qc-pack', '防腐防火质检包', 'PDF + JSON', '质检工程师', ['膜厚记录', '耐火极限', '环境温湿度']),
    ],
    workflowStates: workflow('production_manufacturing'),
    agentGates: gates(),
    tasks: [
      task('pm-t1', '按SS-04-01~SS-04-11生成加工、孔位、BOM和包装工单', '工艺工程师', 'doing'),
      task('pm-t2', '校核CNC孔位与IFC构件GUID、SS-04-05穿梁孔一致性', 'RuleChecker', 'todo'),
      task('pm-t3', '建立无现场焊接的工厂焊接和现场栓接放行清单', '焊接工程师', 'todo'),
    ],
    approvals: [approval('pm-a1', '工厂生产放行审批', '生产负责人')],
    risks: [
      risk('pm-r1', 'CNC 文件与模型版本或孔位锁定不一致', 'critical', '每个 NC/DXF 文件必须绑定 model_revision、element_guid、SS-04-04孔位坐标和审批版本。'),
      risk('pm-r2', '现场误焊破坏全栓接体系', 'critical', '生产包必须声明仅工厂焊接，现场安装按螺栓紧固工艺验收。'),
    ],
    fileTypes: ['.ifc', '.nc', '.dxf', '.step', '.xlsx', '.pdf', '.json'],
    visualization: {
      mode: 'timeline',
      title: 'Q235B全栓接构件生产 HMI',
      layers: ['P1冻结', 'SS-04加工图', '孔位CNC', '工厂焊接', '防腐防火', '包装编码', '质检放行'],
      telemetry: ['p1_release_rate', 'hole_coordinate_conflict', 'cnc_package_count', 'qc_pass_rate', 'shipment_ready_count'],
    },
    standards: ['AWS D1.1', 'EN 1090', 'AS/NZS 5131', 'GB 50205', 'GB 50661'],
    dataObjects: ['production_plans', 'shop_drawing_items', 'bolt_hole_coordinates', 'process_routes', 'cutting_optimizations', 'cnc_files', 'factory_weld_records', 'coating_records', 'fireproofing_records', 'factory_qc', 'mes_sync_jobs', 'component_codes', 'packing_units', 'shipment_batches', 'rework_orders'],
  ...
  },
  {
    ...
    objective: '把吊装顺序、调平校准、高强螺栓紧固、模块拼缝防渗、干湿分区、样板间试装、物料进场和QC安全记录转为可审计 Evidence Token。',
    subdomains: [
      { id: 'hoisting-sequence', name: '钢构吊装顺序', purpose: '柱→主梁→次梁→支撑，先内后外、先下后上，每步稳定性验算。', ownerRole: '现场钢构代表', capabilityLevel: 'automation' },
      { id: 'leveling-calibration', name: '调平标高校准', purpose: '调平螺栓、柱顶水准仪测点、层间累积误差≤±5mm和每层复测。', ownerRole: '测量工程师', capabilityLevel: 'workflow' },
      { id: 'bolt-tightening', name: '高强螺栓紧固', purpose: '初拧50%→终拧100%，从中间向两端扩散，扭矩扳手补拧标记。', ownerRole: '栓接工艺顾问', capabilityLevel: 'workflow' },
      { id: 'module-seam-control', name: '模块拼缝防渗隔声', purpose: 'PU密封胶、48kg/m³矿棉、背衬板和内饰收口按不可逆工序执行。', ownerRole: '防水防渗顾问', capabilityLevel: 'workflow' },
      { id: 'dry-wet-zone', name: '干湿作业分区', purpose: '灌浆、打胶、防水湿作业区与干作业区隔离，控制交叉作业窗口。', ownerRole: '现场精装代表', capabilityLevel: 'workflow' },
      { id: 'mockup-release', name: '样板间试装放行', purpose: '3间样板间端头转角问题清单修正后，作为批量施工放行闸门。', ownerRole: '项目总工', capabilityLevel: 'workflow' },
      { id: 'material-qc-safety', name: '物料质量安全', purpose: '进场核对、关键工序QC、现场安全文明施工、季节性措施和成品保护。', ownerRole: '质检/安全负责人', capabilityLevel: 'automation' },
    ...
    artifacts: [
      artifact('hotel-assembly-evidence-token', '酒店装配施工 Evidence Token', 'JSON', '监理工程师', ['吊装顺序', '调平校准', '螺栓紧固']),
      artifact('bolt-tightening-qc-pack', '高强螺栓紧固QC包', 'PDF + JSON', '栓接工艺顾问', ['初拧/终拧', '扭矩扳手记录', '10%抽检']),
      artifact('mockup-release-pack', '3间样板间试装放行包', 'PDF + BCF', '项目总工', ['问题清单', '修正方案', '批量放行']),
      artifact('site-completion-list', '竣工验收资料清单模板', 'PDF + XLSX', '资料员', ['竣工图', '检验批', '隐蔽验收', '材料合格证']),
    ],
    workflowStates: workflow('construction_management'),
    agentGates: gates(),
    tasks: [
      task('cs-t1', '生成CON-01钢构安装工艺执行和复测记录', '现场钢构代表', 'doing'),
      task('cs-t2', '建立CON-02围护精装施工穿插和样板间放行闸门', '现场精装代表', 'todo'),
      task('cs-t3', '把CON-03物料进场、QC、安全文明和竣工资料清单归档', '资料员', 'todo'),
    ],
    approvals: [approval('cs-a1', '隐蔽工程和关键工序验收', '总监理工程师')],
    risks: [
      risk('cs-r1', '现场证据缺少时间/位置/构件绑定', 'critical', '影像和记录必须绑定 element_id、geo、time、责任人和图纸编号。'),
      risk('cs-r2', '螺栓紧固或调平复测缺失', 'critical', 'CON-01-02、CON-01-03和CON-01-06必须形成可签字验收记录。'),
    ],
    fileTypes: ['.pdf', '.jpg', '.mp4', '.e57', '.las', '.ply', '.bcf', '.ifc', '.json'],
    visualization: {
      mode: 'hmi',
      title: '装配式酒店现场安装与验收 HMI',
      layers: ['吊装顺序', '调平校准', '螺栓紧固', '拼缝防渗', '样板间放行', 'QC清单', '安全文明', '竣工资料'],
      telemetry: ['bolt_torque_pass_rate', 'leveling_deviation_mm', 'mockup_release_status', 'qc_blocker_count', 'evidence_completeness'],
    },
    standards: ['GB 55006', 'GB 50205', 'GB 50300', 'ISO 45001', 'ISO 19650 CDE'],
    dataObjects: ['hoisting_sequences', 'leveling_checks', 'bolt_torque_records', 'module_seam_checks', 'dry_wet_zone_plans', 'mockup_release_records', 'material_receipts', 'qc_checkpoints', 'safety_hazards', 'daily_logs', 'completion_documents'],
  ...
  },
  {
    ...
    objective: '将市场获客、合同佣金、深化图纸、模型版本、孔位锁定、工厂放行、现场验收和客户裂变证据归档为可检索、可审计、可长期保存的 Archive Token。',
    subdomains: [
    ...
    artifacts: [
      ...
      artifact('hotel-drawing-archive-pack', '198份深化图纸归档包', 'ZIP + PDF/A + XLSX', '文控经理', ['8大专项', '33个分组', '图纸编号']),
      artifact('zaofang-marketing-archive-pack', '造房网推广证据归档包', 'ZIP + JSON', '品牌负责人', ['投放记录', '合伙人协议', '样板房接待', '案例传播']),
    ...
    standards: ['ISO 19650 CDE', 'OAIS', 'CJJ/T 117', 'GB/T 50328', 'e-signature audit', '广告与合同证据留存'],
    dataObjects: ['contracts', 'archive_items', 'drawing_catalog_items', 'partner_agreements', 'commission_records', 'marketing_media_assets', 'retention_policies', 'signature_events', 'handover_packages'],
  ...
  },
  {
    ...
    track: 'cost',
    status: 'planned',
    summary: '覆盖造房网合伙人佣金、样板房预算、客户签约回款、项目合同、收付款、发票、成本、班组绩效和组织能力。',
    objective: '把 5% 合伙人佣金、回款后7个工作日结算、样板房55万-74万元投入、项目生产成本和签约后服务转成可核算、可付款、可审计的 Enterprise Token。',
    subdomains: [
      { id: 'contract-ledger', name: '合同台账', purpose: '管理主合同、分包合同、补充协议、履约节点和合同风险。', ownerRole: '商务经理', capabilityLevel: 'workflow' },
      { id: 'partner-commission', name: '合伙人佣金', purpose: '管理5%佣金、回款后7个工作日结算和推荐奖励。', ownerRole: '渠道财务', capabilityLevel: 'workflow' },
      { id: 'sample-house-budget', name: '样板房预算', purpose: '跟踪146㎡占地、224㎡建筑面积、55万-74万元展示样板房预算拆解。', ownerRole: '财务经理', capabilityLevel: 'workflow' },
    ...
    artifacts: [
      ...
      artifact('partner-commission-ledger', '合伙人佣金结算台账', 'XLSX + JSON', '渠道财务', ['合伙人协议', '客户回款', '7个工作日SLA']),
      artifact('sample-house-budget-pack', '样板房预算拆解包', 'XLSX + PDF', '财务经理', ['主体30万', '基础5万', '设备5万', '精装15万']),
    ...
    agentGates: gates(),
    tasks: [
      task('fh-t1', '建立合伙人佣金结算与客户回款绑定关系', '渠道财务', 'doing'),
      task('fh-t2', '同步样板房预算、标杆案例投入和签约转化成本', '财务经理', 'todo'),
      task('fh-t3', '同步班组考勤、质量安全和绩效记录', '人力资源经理', 'todo'),
    ],
    ...
    standards: ['PMBOK cost management', 'ISO 9001 records', 'SOC 2 audit trail', 'local tax invoice rules', '佣金结算与反舞弊内控'],
    dataObjects: ['contracts', 'partner_commission_ledgers', 'sample_house_budgets', 'payment_requests', 'invoices', 'cashflow_items', 'cost_ledgers', 'employees', 'crews', 'attendance_records', 'performance_scores'],
