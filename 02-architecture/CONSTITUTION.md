...
`ArchIToken` 是历史项目名、仓库名、路径名、包名/API 兼容标识和迁移期技术别名。新用户界面、对外文档、桌面入口、发行物和新增功能命名默认使用 `ArchIToken`;只有历史路径、兼容合同、迁移说明或引用既有文件名时才使用 `ArchIToken`。本文件中尚未迁移的 `ArchIToken` 表述在迁移期按 `ArchIToken` 理解,除非上下文明示为历史名称。
...
个人中心 -> 市场客服 -> 方案设计 -> 深化设计 -> 计量造价 -> 生产制造 -> 材料物流 -> 施工管理 -> 数字孪生 -> 数字档案 -> 财务管理 / 人力资源 -> AI 中心 / 设置中心,必须通过数据库记录、CDE 文件、审计事件、Router 和审批状态串联。任何静态页面、假数据、只前端状态或无审计链实现都不能被标记为生产完成。
...
- Prengine 是 ArchIToken 自研工程智能、几何、属性、资产派生和协同调用引擎名称,不是第三方 viewer 名称,也不是单一前端组件名称。Prengine 能力必须覆盖 OpenEngineeringEditor、AI 生成、可编辑几何、可编辑属性、构件选择、尺寸量测、坐标/单位、BOM/清单导出、worker 派生、审计、SDK 和 API 调用。OpenEngineeringViewer 只能作为兼容别名或只读模式名称,不得作为工程模型能力边界。
