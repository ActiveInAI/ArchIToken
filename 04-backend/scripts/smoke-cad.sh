...
job="$(create_phase7_conversion_job "cad_extract_entities" "${asset_id}" "${file_id}" '{"cad":["dxf_extract_entities","occt_adapter_boundary"],"dwg":"legal_adapter_boundary_only"}')"
printf '%s\n' "${job}" | jq -e '.operation == "cad_extract_entities" and .status == "queued" and .input.dwg == "legal_adapter_boundary_only"' >/dev/null
