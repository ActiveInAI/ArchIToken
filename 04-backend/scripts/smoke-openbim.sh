...
printf '%s\n' "${job}" | jq -e '.operation == "ifc_to_3dtiles" and (.status == "queued" or .status == "dispatched") and .input.openbim[0] == "IFC4x3"' >/dev/null
...
printf '%s\n' "${ifcdb_job}" | jq -e '.operation == "ifcdb_query" and (.status == "queued" or .status == "dispatched") and .input.adapter == "ifcdb_agent"' >/dev/null
