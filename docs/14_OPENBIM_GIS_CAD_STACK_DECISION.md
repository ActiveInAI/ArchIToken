...
RVT/RFA, SketchUp SKP, Rhino 3DM, and unknown vendor BIM sources such as STEL route through the `licensed_bim_adapter` boundary. Production success requires real source bytes plus persisted IFC/GLB/STEP derivatives or object-store references; the gateway and workers must not claim support from placeholder manifests. SKP->IFC specifically requires a real legal SKP reader/exporter command such as `PANAEC_SKP_TO_IFC_COMMAND` or a licensed service, and must not be satisfied by GLB viewing fallback. 3DM->IFC similarly requires `PANAEC_3DM_TO_IFC_COMMAND` or `RHINO_ADAPTER_URL`/`LICENSED_BIM_ADAPTER_URL`; Rhino display meshes, browser Three.js objects and source inspection views must not be synthesized into IFC.
