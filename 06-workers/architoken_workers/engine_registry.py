...
ENGINE_POLICIES: dict[str, EnginePolicy] = {
    ...
    "huggingface_media": EnginePolicy(
        "huggingface_media",
        "Hugging Face local media runtime",
        "https://huggingface.co/models",
        "model-specific license review required",
        IsolationMode.SIDECAR_SERVICE,
        "Text/image/video generation must run through a configured local HTTP or command adapter; cached model files alone are not executable provider readiness.",
        (
    ...
    "blender_plugin": EnginePolicy(
        "blender_plugin",
        "Blender add-on and extension runtime",
        "https://docs.blender.org/manual/en/latest/advanced/extensions/index.html",
        "extension-specific license review required",
        IsolationMode.EXTERNAL_PROCESS,
        "Audit every add-on or extension package first, then install and execute only inside a job-scoped Blender user profile with explicit workflow approval.",
        ("BLENDER_BINARY", "BLENDER_USER_EXTENSIONS", "ARCHITOKEN_BLENDER_EXTENSION_REPOSITORY_URL"),
        True,
    ),
    "blender_plugin_run": EnginePolicy(
        "blender_plugin_run",
        "Blender add-on and extension execution runtime",
        "https://docs.blender.org/api/current/bpy.ops.preferences.html",
        "extension-specific license review required",
        IsolationMode.EXTERNAL_PROCESS,
        "Execute approved Blender add-ons/extensions only in an ephemeral worker profile and persist run manifests plus scene derivatives.",
        ("BLENDER_BINARY",),
        True,
    ),
    "bcf": EnginePolicy(
    ...
    "openbim_evidence": EnginePolicy(
        "openbim_evidence",
        "buildingSMART/openBIM evidence report recorder",
        "https://www.buildingsmart.org/compliance/software-certification/ifc/",
        "standard",
        IsolationMode.IN_PROCESS_LIBRARY,
        "Persist real full-chain sample, OpenCDE/API contract, and official certification/conformance evidence without inventing compliance.",
    ),
    ...
    "chart_spec": EnginePolicy(
        "chart_spec",
        "D3 / ECharts-compatible specs",
        "https://github.com/antvis/G2",
        "MIT",
        IsolationMode.IN_PROCESS_LIBRARY,
        "Persist typed chart specs only; frontend rendering uses reviewed visualization packages.",
    ),
    "component_bom": EnginePolicy(
        "component_bom",
        "ArchIToken component BOM XLSX importer",
        "internal://architoken-component-bom-worker",
        "Apache-2.0",
        IsolationMode.IN_PROCESS_LIBRARY,
        "Import only mounted XLSX source workbooks, preserve source rows/cell ranges, and mark all BOM output as professional_review_required until approval.",
    ),
    ...
    "ddc_converter": EnginePolicy(
        "ddc_converter",
        "DataDrivenConstruction converter packages",
        "https://pkg.datadrivenconstruction.io",
        "proprietary DDC/ODA runtime packages",
        IsolationMode.EXTERNAL_PROCESS,
        "Run DDC converter binaries only as external processes and persist real XLSX/DAE/IFC artifacts; SKP is unavailable unless a licensed SKP adapter is configured.",
        (
            "DgnExporter",
            "DwgExporter",
            "IfcExporter",
            "RvtExporter",
            "RVT2IFCconverter",
            "DDC_DGN_EXPORTER_PATH",
            "DDC_DWG_EXPORTER_PATH",
            "DDC_IFC_EXPORTER_PATH",
    ...
    "floorplan_layout": EnginePolicy(
        "floorplan_layout",
        "ArchIToken floorplan-layout kernel",
        "internal://architoken/floorplan-layout",
        "Apache-2.0",
        IsolationMode.IN_PROCESS_LIBRARY,
        "Generate only structured layout manifests and review-required evaluation reports; do not mark plans compliant or construction-ready.",
    ),
    "steel_platform": EnginePolicy(
        "steel_platform",
        "ArchIToken steel-platform detailed-design kernel",
        "user-archive://steel-platform-full.zip",
        "user-supplied archive integration; runtime code is clean-room ArchIToken worker",
        IsolationMode.IN_PROCESS_LIBRARY,
        "Accept only structured 2D plan blocks and steel-platform settings; persist review-required design packages/BOM, and generate STEP/STL/glTF only through real build123d/OCP when installed.",
        ("build123d", "OCP"),
    ),
    ...
    "officecli": EnginePolicy(
        "officecli",
        "OfficeCLI",
        "https://github.com/iOfficeAI/OfficeCLI",
        "Apache-2.0",
        IsolationMode.EXTERNAL_PROCESS,
        "Run OfficeCLI only as an isolated worker binary for structured DOCX/XLSX/PPTX generation, editing, and preview artifacts.",
    ...
    "mineru": EnginePolicy(
        ...
        "Apache-2.0 + MinerU additional terms",
        IsolationMode.EXTERNAL_PROCESS,
        "Run MinerU 3.1.15+ as an isolated CLI/container for PDF/Office document-intelligence extraction.",
        ("mineru",),
        True,
    ),
    "mupdf": EnginePolicy(
    ...
    "paddleocr": EnginePolicy(
        ...
        "Apache-2.0",
        IsolationMode.IN_PROCESS_LIBRARY,
        "Run PaddleOCR/PaddleOCR-VL/PP-StructureV3 only on real source PDFs/images for OCR, layout and document-vision extraction; persist extracted blocks as evidence, not edit success.",
    ...
    "stirling_pdf": EnginePolicy(
        ...
        "MIT core + separate proprietary/saas/engine subdirectory licenses",
        IsolationMode.SIDECAR_SERVICE,
        "Run as a self-hosted PDF service for registered PDF operations; call only configured or Swagger-verified API paths and persist real derivatives/evidence.",
        ("STIRLING_PDF_URL",),
    ),
}
